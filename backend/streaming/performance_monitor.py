...
class StreamingPerformanceMonitor:
    ...
    def record_message(self, latency_ms: float, quality_score: float, stream_name: str):
        ...
        self._record_metric(
            f"stream_{stream_name}_latency_ms", latency_ms, "ms", timestamp
        )
        self._record_metric(
            f"stream_{stream_name}_quality", quality_score, "score", timestamp
        )
    ...
    def record_error(self, error_type: str, stream_name: str):
        ...
        self._record_metric(
            f"errors_{error_type}", 1, "count", timestamp, {"stream": stream_name}
        )
    ...
    def record_stream_metrics(self, stream_name: str, metrics: dict[str, Any]):
        ...
        for metric_name, value in metrics.items():
            if isinstance(value, int | float):
                self._record_metric(
                    f"stream_{stream_name}_{metric_name}", value, "auto", timestamp
                )
            ...
            if metric_name in self.metrics and self.metrics[metric_name]:
                ...
                if self._is_threshold_breached(metric_name, latest.value, threshold):
                    ...
                    if metric_name in self.alert_cooldown:
                        if timestamp - self.alert_cooldown[metric_name] < timedelta(
                            minutes=5
                        ):
                            continue
                    ...
                    severity = (
                        "critical"
                        if self._is_critical(metric_name, latest.value)
                        else "warning"
                    )
                    ...
                    alert = PerformanceAlert(
                        ...
                        message=self._get_alert_message(
                            metric_name, latest.value, threshold
                        ),
                    )
    ...
    def _get_alert_message(
        ...
        return messages.get(
            metric_name, f"{metric_name} is {value} (threshold: {threshold})"
        )
    ...
    def get_current_metrics(self) -> dict[str, Any]:
        ...
        metrics["summary"] = {
            ...
            "overall_error_rate": (
                self.total_errors / self.total_messages
                if self.total_messages > 0
                else 0
            ),
        }
    ...
    def _generate_recommendations(
        self, current_metrics: dict[str, Any], stats: dict[str, Any]
    ) -> list[str]:
    ...
    def export_metrics(self, filepath: str, format: str = "json"):
        ...
        try:
            ...
            for metric_name in ["latency_p95_ms", "messages_per_second", "error_rate"]:
                data["metric_history"][metric_name] = self.get_metric_history(
                    metric_name, 3600
                )

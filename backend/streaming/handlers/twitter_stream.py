...
class TwitterStreamHandler(DataStreamHandler):
    ...
    async def subscribe(self, symbols: list[str]):
        ...
        try:
            ...
            for symbol in symbols:
                ...
                rule_value = (
                    f"(${symbol} OR {symbol}) (stock OR trading OR market OR price)"
                )

                rules.append({"value": rule_value, "tag": f"symbol:{symbol}"})
            ...
            try:
                ...
                async with self.session.get(
                    ...
                    if response.status == 429:
                        ...
                        logger.warning(
                            f"Twitter rate limited, sleeping for {sleep_time}s"
                        )
    ...
    async def _process_tweet(self, line: bytes):
        ...
        try:
            ...
            quality_score = min(
                1.0, (engagement_score * 0.5) + (author_influence * 0.5)
            )
    ...
    async def _clear_rules(self):
        ...
        try:
            ...
            async with self.session.get(self.rules_url) as response:
                ...
                async with self.session.post(
                    self.rules_url, json={"delete": {"ids": rule_ids}}
                ) as delete_response:

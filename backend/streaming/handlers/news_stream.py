...
class NewsStreamHandler(DataStreamHandler):
        ...
        try:
            ...
            for source_name, source_config in self.news_sources.items():
                if source_name not in self.polling_tasks:
                    ...
                    if (
                        source_config.get("requires_key")
                        and source_name not in self.api_keys
                    ):
    ...
    async def _poll_news_source(self, source_name: str, config: dict[str, Any]):
        ...
        while self.status == StreamStatus.CONNECTED:
            try:
                ...
                if new_articles > 0:
                    logger.info(
                        f"Processed {new_articles} new articles from {source_name}"
                    )
    ...
    async def _process_article(self, article: dict[str, Any], source_name: str) -> bool:
        ...
        try:
            ...
            article_id = hashlib.md5(
                f"{article['title']}:{article['link']}".encode(), usedforsecurity=False
            ).hexdigest()
    ...
    def _calculate_article_quality(
        self, article: dict[str, Any], source_name: str
    ) -> float:

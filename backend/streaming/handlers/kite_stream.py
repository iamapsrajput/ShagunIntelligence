...
class KiteStreamHandler(DataStreamHandler):
    ...
    async def process_message(self, message: Any) -> StreamMessage | None:
        ...
        try:
            ...
            tick_data = await asyncio.wait_for(self.message_queue.get(), timeout=1.0)
    ...
    async def _process_tick(self, tick: dict[str, Any]) -> StreamMessage | None:
        ...
        try:
            ...
            self.metrics.average_latency_ms = (
                self.metrics.average_latency_ms * (self.metrics.messages_processed - 1)
                + latency_ms

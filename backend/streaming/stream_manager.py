...
from .realtime_pipeline import RealTimeDataPipeline, StreamConfig
...
class StreamManager:
    ...
    async def _setup_streams(self):
        ...
        if self.config.enable_alpha_vantage and self.config.alpha_vantage_api_key:
            ...
            av_handler = AlphaVantageStreamHandler(
                config=av_config, api_key=self.config.alpha_vantage_api_key
            )
        ...
        if self.config.enable_finnhub and self.config.finnhub_api_key:
            ...
            finnhub_handler = FinnhubStreamHandler(
                config=finnhub_config, api_key=self.config.finnhub_api_key
            )
        ...
        if self.config.enable_twitter and self.config.twitter_bearer_token:
            ...
            twitter_handler = TwitterStreamHandler(
                config=twitter_config, bearer_token=self.config.twitter_bearer_token
            )
        ...
        if self.config.enable_news:
            ...
            news_handler = NewsStreamHandler(config=news_config, api_keys=api_keys)
    ...
    async def subscribe_symbols(self, symbols: list[str]):
        ...
        try:
            ...
            for stream_name, _handler in self.pipeline.streams.items():
            ...
            for symbol in symbols:
                primary_stream = await self.pipeline._find_best_stream_for_symbol(
                    symbol
                )
                if primary_stream:
    ...
    async def get_latest_data(self, symbol: str) -> dict[str, Any] | None:
            ...
            if handler.status.value == "connected":
                ...
                if stream_buffer in self.pipeline.data_buffers:
                    ...
                    if buffer:
                        latest = buffer[-1]
                        stream_data = {
                            "price": latest.data.get("price")
                            or latest.data.get("last_price"),
    ...
    async def get_historical_buffer(
        self, symbol: str, seconds: int = 60, stream: str | None = None
    ) -> list[dict[str, Any]]:
        """Get historical data from buffer."""
        if stream:
            # Get from specific stream
            messages = await self.pipeline.get_historical_buffer(
                f"{symbol}:{stream}", seconds
            )
        else:
    ...
    def get_pipeline_metrics(self) -> dict[str, Any]:
        ...
        if metrics["total_messages"] > 0:
            uptime = sum(
                h.metrics.uptime_seconds for h in self.pipeline.streams.values()
            ) / len(self.pipeline.streams)

            if uptime > 0:
                metrics["messages_per_minute"] = metrics["total_messages"] / (
                    uptime / 60
                )

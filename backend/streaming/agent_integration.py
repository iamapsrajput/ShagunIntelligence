...
from .realtime_pipeline import StreamMessage
...
class StreamingAgentBridge:
    ...
    async def _transform_data_for_agent(
        self, agent_id: str, agent_type: str, message: StreamMessage
    ) -> dict[str, Any]:
    ...
    def _transform_for_technical_analyst(
    ...
    def _transform_for_sentiment_analyst(
        self, message: StreamMessage
    ) -> dict[str, Any]:
    ...
    def _calculate_spread(self, data: dict[str, Any]) -> float | None:
    ...
    def _determine_priority(self, message: StreamMessage, agent_type: str) -> str:
        ...
        if (
            message.data.get("type") == "news"
            and "breaking" in message.data.get("title", "").lower()
        ):
            return "high"
    ...
    async def get_agent_data_summary(self, agent_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "last_update": (
                sub["last_update"].isoformat() if sub["last_update"] else None
            ),
            "active": (
                (datetime.now() - sub["last_update"]).seconds < 60
                if sub["last_update"]
                else False
            ),
        }

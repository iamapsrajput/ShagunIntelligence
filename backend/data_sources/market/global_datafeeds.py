...
class GlobalDatafeedsSource(MarketDataSource):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            auth_data = {"api_key": self.api_key, "user_id": self.user_id}
    ...
    async def get_quote(self, symbol: str) -> dict[str, Any]:
        ...
        try:
            ...
            parts = symbol.split(".")
            ticker = parts[0]
            exchange = self.exchange_map.get(
                parts[1] if len(parts) > 1 else "NSE", "NSE_EQ"
            )

            url = f"{self.base_url}/realtime/quote"
            params = {"api_key": self.api_key, "symbol": ticker, "exchange": exchange}
    ...
    async def get_quotes(self, symbols: list[str]) -> dict[str, dict[str, Any]]:
        ...
        for i in range(0, len(symbols), batch_size):
            ...
            try:
                ...
                params = {"api_key": self.api_key, "symbols": ",".join(symbol_list)}
                    ...
                    if response.status == 200:
                        ...
                        if data.get("success"):
                            ...
                            for symbol, quote_data in quotes_data.items():
                                ...
                                original_symbol = (
                                    f"{ticker}.{self._reverse_exchange_map(exchange)}"
                                )

...
class MarketSourceManager:
    ...
    def __init__(self, strategy: SourceSelectionStrategy = SourceSelectionStrategy.BALANCED):
        ...
        for source_name in sources:
            if self._is_source_available(source_name):
                try:
                    ...
                    logger.error(f"Error fetching historical data from {source_name}: {e}")
                    self._track_request_failure(source_name)
                    continue
    ...
    async def stream_quotes(
        ...
        for market, market_symbols in market_groups.items():
            ...
            for source_name in sources:
                if (self._is_source_available(source_name) and 
                    self.feature_support[source_name].get("streaming", False)):
                    try:
                        source = self.sources[source_name]
                        ...
                        logger.info(f"Started streaming {len(market_symbols)} symbols from {source_name}")
                        break
    ...
    async def get_technical_indicators(
        ...
        **kwargs
        ...
        if "alpha_vantage" in self.sources and self._is_source_available("alpha_vantage"):
            try:
    ...
    def _select_sources_for_request(
        self,
        market: str,
        request_type: str
    ) -> List[str]:
        ...
        available_sources = [
            source for source, markets in self.market_coverage.items()
        ...
        if self.strategy == SourceSelectionStrategy.COST_OPTIMIZED:
            ...
            available_sources.sort(key=lambda s: (
            ...
            available_sources.sort(
                key=lambda s: -self.quality_scores.get(s, 0)
            )
            
        elif self.strategy == SourceSelectionStrategy.LATENCY_OPTIMIZED:
            # Sort by latency
            available_sources.sort(
                key=lambda s: self.latency_estimates.get(s, 999)
            )
            ...
            def balanced_score(source):
                ...
                cost = 1 if self.sources[source].get_cost_info().monthly_cost == 0 else 0.5
    ...
    def get_source_status(self) -> Dict[str, Any]:
        ...
        for source_name, source in self.sources.items():
            ...
            status[source_name] = {
                "health": self.source_health.get(source_name, DataSourceStatus.UNKNOWN).value,

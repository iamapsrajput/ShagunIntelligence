...
class PolygonSource(MarketDataSource):
            ...
            async with self.session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    if "status" in data:
                        ...
                        logger.info(f"Connected to Polygon. Market status: {data.get('market')}")
                        return True
                    else:
    ...
    async def _check_rate_limit(self):
        ...
        # Check limit
        if self.cost_info.requests_per_minute and len(self.request_times) >= self.cost_info.requests_per_minute:
                ...
                if response.status == 200:
                    ...
                    if data.get("status") == "OK":
                        ...
                        market_data = MarketData(
                            ...
                            timestamp=datetime.fromtimestamp(trade.get("t", 0) / 1000000000),  # Nanoseconds
                            source="polygon",
                            ...
                            is_delayed=self.cost_info.tier == DataCostTier.FREE  # Free tier is delayed
                        )
    ...
    async def _get_last_quote(self, symbol: str) -> Optional[Dict[str, Any]]:
    ...
    async def _get_daily_aggregate(self, symbol: str) -> Optional[Dict[str, Any]]:
    ...
    async def _get_grouped_quotes(self, symbols: List[str]) -> Dict[str, Dict[str, Any]]:
                    ...
                    for item in data:
                        ...
                        if ev_type == "T":  # Trade
                            ...
                            market_data = MarketData(
                                ...
                                timestamp=datetime.fromtimestamp(item.get("t", 0) / 1000000000),
                                source="polygon",
                            ...
                            quote_update = {
                                ...
                                "timestamp": datetime.fromtimestamp(item.get("t", 0) / 1000000000).isoformat(),
                                "type": "quote_update"
                            }
    ...
    async def get_options_chain(self, underlying: str, expiration: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def get_crypto_quote(self, symbol: str, exchange: str = "X:BTCUSD") -> Dict[str, Any]:
                    ...
                    if data.get("status") == "OK":
                        ...
                        for ticker in tickers:
                            ...
                            snapshot["indices"][symbol] = {
                                ...
                                "change": float(day.get("c", 0)) - float(day.get("o", 0)),
                                "change_percent": ((float(day.get("c", 0)) - float(day.get("o", 0))) / float(day.get("o", 1))) * 100,

...
class MarketauxNewsSource(NewsDataSource):
        ...
        try:
            async with self.session.get(url, params=params) as response:
                ...
                for item in data.get("data", []):
                    article = self._parse_news_item(item, symbols or [])
                    if article and article.market_impact in [MarketImpact.HIGH, MarketImpact.MEDIUM]:
                        articles.append(article)
    ...
    def _parse_news_item(self, item: Dict[str, Any], requested_symbols: List[str]) -> Optional[NewsArticle]:
        ...
        try:
            ...
            article = NewsArticle(
                ...
                content=item.get("description", ""),  # Marketaux doesn't provide full content
                url=url,
    ...
    def _analyze_market_impact(self, article: NewsArticle, item: Dict[str, Any]) -> MarketImpact:
        """Analyze market impact using Marketaux data"""
        # Check highlights for impact indicators
        highlights = item.get("highlights", [])
        
        high_impact_highlights = ["earnings", "merger", "acquisition", "bankruptcy", "investigation"]
    ...
    def _map_categories_to_industries(self, categories: List[NewsCategory]) -> List[str]:

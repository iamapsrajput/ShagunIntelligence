...
class NewsAggregator:
    ...
    async def initialize_sources(self):
        ...
        # Alpha Vantage (temporarily disabled due to abstract method issues)
        ...
        #         logger.warning(f"Failed to initialize Alpha Vantage news: {e}")

        # EODHD (temporarily disabled due to abstract method issues)
        ...
        #     except Exception as e:
        #         logger.warning(f"Failed to initialize EODHD news: {e}")

        # Marketaux (temporarily disabled due to abstract method issues)
        ...
        #     except Exception as e:
        #         logger.warning(f"Failed to initialize Marketaux news: {e}")
        ...
        while self.monitored_symbols:
            try:
                ...
                recent_news = await self.fetch_breaking_news(
                    list(self.monitored_symbols), minutes=5
                )

                # Check for significant news
                for article in recent_news:
                    if article.market_impact in [
                        MarketImpact.HIGH,
                        MarketImpact.MEDIUM,
                    ]:
    ...
    def _deduplicate_articles(self, articles: list[NewsArticle]) -> list[NewsArticle]:
        ...
        for _hour, group in time_groups.items():
            # Sort by reliability (prefer better sources)
            group.sort(
                key=lambda x: self.reliability_tracker.get_reliability_score(
                    x.source_domain
                ),
                reverse=True,
            )
    ...
    def _calculate_similarity(
        ...
        # Weighted similarity
        similarity = title_sim * 0.6 + time_sim * 0.2 + symbol_sim * 0.2
    ...
    def _apply_reliability_weighting(
    ...
    def get_source_stats(self) -> dict[str, Any]:
        ...
        stats = {
            ...
            "active_sources": sum(
                1 for s in self.sources.values() if s.health.status.value == "healthy"
            ),
            "source_health": {
                name: {

...
from .base import NewsDataSource, NewsArticle, NewsSentiment, NewsCategory, MarketImpact
...
class EODHDNewsSource(NewsDataSource):
    ...
    async def fetch_news(
        ...
        try:
            ...
            if symbols:
                ...
                general_articles = await self._fetch_general_news(
                    from_date, to_date
                )
    ...
    def _parse_news_item(self, item: Dict[str, Any], requested_symbols: List[str]) -> Optional[NewsArticle]:
        ...
        try:
            ...
            # Calculate relevance
            article.relevance_score = self.calculate_relevance_score(article, requested_symbols)
    ...
    def _analyze_content_sentiment(self, article: NewsArticle):
        ...
        very_bullish_keywords = [
            "soar", "surge", "skyrocket", "breakthrough", "record high",
            "exceptional", "outstanding", "massive growth"
        ]
        bullish_keywords = [
            "rise", "gain", "increase", "positive", "growth", "profit",
            "beat expectations", "upgrade", "strong", "improve"
        ]
        bearish_keywords = [
            "fall", "drop", "decline", "negative", "loss", "miss",
            "downgrade", "weak", "concern", "risk"
        ]
        very_bearish_keywords = [
            "crash", "plunge", "collapse", "bankruptcy", "crisis",
            "investigation", "fraud", "disaster"
        ]
        
        # Count sentiment indicators
        very_bullish_count = sum(1 for keyword in very_bullish_keywords if keyword in content)
        ...
        very_bearish_count = sum(1 for keyword in very_bearish_keywords if keyword in content)

...
class SymbolSentiment:
    ...
    def get_aggregate_sentiment(self, hours: int = 1) -> Dict[str, Any]:
        ...
        volume_confidence = min(len(recent_tweets) / 50, 1.0)  # Max confidence at 50 tweets
...
class TwitterSentimentSource(SentimentDataSource):
    ...
    def __init__(self, config: DataSourceConfig):
        ...
        self.financial_keywords = {
            ...
            "calls", "puts", "moon", "dump", "rally", "crash",
            "breakout", "resistance", "support", "earnings",
    ...
    async def _setup_stream_rules(self) -> None:
        ...
        if hasattr(self, '_tracked_symbols') and self._tracked_symbols:
            for symbol in self._tracked_symbols:
                rules.append({
                    "value": f"${symbol} OR #{symbol}",
                    "tag": f"symbol_{symbol}"
                })
        
        # Add general financial keywords rule
        keywords = " OR ".join(self.financial_keywords)
        rules.append({
            "value": f"({keywords}) lang:en -is:retweet",
    ...
    def _analyze_sentiment(self, text: str) -> Tuple[float, str, float]:
        ...
        try:
            ...
            base_confidence = 1.0 - (subjectivity * 0.5)  # Less subjective = more confident
            keyword_confidence = min(abs(sentiment_boost) * 2, 0.3)  # Boost for financial keywords
    ...
    def _calculate_keyword_boost(self, text: str) -> float:
        ...
        bullish_keywords = {"bullish", "buy", "long", "calls", "moon", "rally", "breakout", "upgrade"}
        bearish_keywords = {"bearish", "sell", "short", "puts", "dump", "crash", "resistance", "downgrade"}
    ...
    async def _calculate_user_influence(
        ...
        # Check cache
        cache_key = f"{author_id}_{int(datetime.utcnow().timestamp() / self.user_cache_ttl)}"
        ...
        if user_data and "public_metrics" in user_data:
            ...
            if followers > 0:
                ...
                follower_score = min(np.log10(followers + 1) / 6, 1.0)  # Max at 1M followers
...
class TwitterRateLimiter:
    ...
    async def check_limit(self, endpoint: str) -> None:
        ...
        if len(self.calls[endpoint]) >= limit["calls"]:
            wait_time = (self.calls[endpoint][0] + timedelta(seconds=limit["window"]) - now).total_seconds()
            if wait_time > 0:
                logger.warning(f"Rate limit reached for {endpoint}, waiting {wait_time}s")

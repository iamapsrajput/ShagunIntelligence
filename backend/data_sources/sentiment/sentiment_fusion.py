...
from backend.data_sources.sentiment.twitter_api import TwitterSentimentSource
...
class MultiSourceSentimentFusion:
    ...
    async def get_sentiment_trends(
        ...
        for name, sentiment_source in self.sources.items():
            try:
                if hasattr(sentiment_source.source, 'get_sentiment_trends'):
                    result = await sentiment_source.source.get_sentiment_trends(symbol, hours)
        ...
        for hour, data in sorted(hourly_data.items()):
            if data["scores"]:
                ...
                if total_weight > 0:
                    weighted_score = sum(
                        s["score"] * s["weight"] for s in data["scores"]
                    ) / total_weight
    ...
    async def get_real_time_alerts(
        ...
        if abs(sentiment["fused_sentiment_score"]) > threshold:
            alert_type = "bullish" if sentiment["fused_sentiment_score"] > 0 else "bearish"
    ...
    async def backtest_fusion_accuracy(
        ...
        for sentiment in trends["trends"]:
            ...
            for i, price in enumerate(price_data):
                ...
                if abs((price_time - sentiment_time).total_seconds()) < 3600:  # Within 1 hour
        ...
        correct_predictions = sum(
            1 for d in aligned_data
            if (d["sentiment"] > 0 and d["price_change"] > 0) or
               (d["sentiment"] < 0 and d["price_change"] < 0)
        )

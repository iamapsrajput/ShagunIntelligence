...
class GrokCostManager:
    ...
    def record_usage(self, tokens: int, cost: float = None):
        ...
        self.request_history.append(
            {"timestamp": datetime.utcnow(), "tokens": tokens, "cost": cost}
        )
    ...
    def get_usage_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "requests_today": len(
                [r for r in self.request_history if r["timestamp"].date() == today]
            ),
        }
...
class GrokBatchProcessor:
    ...
    async def add_request(self, symbol: str, callback: Any, priority: int = 5) -> None:
        """Add request to batch queue"""
        async with self.batch_lock:
            self.pending_requests[priority].append(
                {"symbol": symbol, "callback": callback, "timestamp": datetime.utcnow()}
            )
    ...
    async def _process_batches(self) -> None:
        ...
        async with self.batch_lock:
            ...
            self.pending_requests = defaultdict(
                list, {k: v for k, v in self.pending_requests.items() if v}
            )
...
class GrokSentimentSource(SentimentDataSource):
    ...
    def __init__(self, config: DataSourceConfig):
        ...
        self.api_url = config.credentials.get(
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            test_response = await self._make_request("Test connection", max_tokens=10)
    ...
    async def analyze_batch(
        ...
        for symbol in symbols:
            cache_key = (
                f"{symbol}_{int(datetime.utcnow().timestamp() / self.cache_ttl)}"
            )
            if use_cache and cache_key in self.sentiment_cache:
        ...
        batches = [
            uncached_symbols[i : i + 10] for i in range(0, len(uncached_symbols), 10)
        ]
                ...
                try:
                    ...
                    for symbol, symbol_data in symbols_data.items():
                        sentiment_response = GrokSentimentResponse(
                            symbol=symbol,
                            sentiment_score=float(
                                symbol_data.get("sentiment_score", 0)
                            ),
                            sentiment_label=symbol_data.get(
                            ...
                            key_factors=[],
                            trend_direction=symbol_data.get(
    ...
    async def get_social_sentiment(
        self, symbol: str, platform: str = "x"
    ) -> dict[str, Any]:
    ...
    async def get_sentiment_trends(
        ...
        try:
            ...
            return {"symbol": symbol, "trends": [], "source": "grok", "error": str(e)}

...
class DataSourceIntegration:
    ...
    def __init__(self):
        ...
        self.market_source_manager = MarketSourceManager(SourceSelectionStrategy.BALANCED)
        ...
        try:
            ...
            if bearer_token:
                ...
                if success:
                    ...
                    self.sentiment_fusion.add_source("twitter", self._twitter_source, weight=1.0)
        ...
        try:
            ...
            if grok_api_key:
                ...
                if success:
                    ...
                    self.sentiment_fusion.add_source("grok", self._grok_source, weight=1.5)
    ...
    async def get_sentiment_alerts(self, symbol: str, threshold: float = 0.3) -> List[Dict[str, Any]]:
        ...
        try:
            if self.sentiment_fusion.sources:
                return await self.sentiment_fusion.get_real_time_alerts(symbol, threshold)
            else:
                return []
        except Exception as e:
    ...
    async def get_grok_analysis(self, symbol: str) -> Dict[str, Any]:
        ...
        if not self._grok_source:
            return {
                "error": "Grok sentiment source not initialized"
            }
    ...
    async def get_news_sentiment_summary(
        ...
        sentiment_dist = {
            sentiment.value: 0 for sentiment in NewsSentiment
        }
        ...
        avg_impact = sum(
            impact_scores.get(article.market_impact, 1)
            for article in articles
        ) / len(articles)
    ...
    async def force_health_check(self, source_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    def get_active_source(self, source_type: str = 'market_data') -> Optional[str]:
    ...
    async def stream_market_data(self, symbols: List[str], callback: Any) -> Optional[asyncio.Task]:
    ...
    async def get_technical_indicators(self, symbol: str, indicator: str, **kwargs) -> Optional[Dict[str, Any]]:
        """Get technical indicators from Alpha Vantage."""
        return await self.market_source_manager.get_technical_indicators(symbol, indicator, **kwargs)


# Singleton instance
_integration: Optional[DataSourceIntegration] = None

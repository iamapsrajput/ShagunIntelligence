...
from backend.data_sources.models import MarketData, DataSourceType
...
class DataQualityValidator:
    def __init__(
        ...
        self.min_required_fields = min_required_fields or [
            "symbol", "current_price", "volume", "timestamp"
        ]
    ...
    def validate_data(
        ...
        reference_data: Optional[Dict[str, MarketData]] = None
    ) -> QualityMetrics:
        """
    ...
    def _calculate_completeness_score(self, data: MarketData) -> float:
        ...
        required_present = sum(
            1 for field in self.min_required_fields
        ...
        optional_fields = [
            "bid", "ask", "high", "low", "open", "close",
            "bid_size", "ask_size", "vwap", "market_cap"
        ]
    ...
    def _calculate_accuracy_score(
        self,
        data: MarketData,
        reference_data: Dict[str, MarketData]
    ...
    def _detect_price_anomalies(self, data: MarketData) -> List[str]:
        ...
        if history:
            ...
            if price_change > 0.1:  # More than 10% change
                anomalies.append(
                    f"Sudden price change: {price_change:.2%}"
                )
    ...
    def _detect_volume_anomalies(self, data: MarketData) -> List[str]:
        ...
        if median_volume > 0:
            ...
            if volume_ratio > self.volume_spike_threshold:
                anomalies.append(
                    f"Volume spike: {volume_ratio:.2f}x median"
                )
            elif volume_ratio < 0.1:  # Very low volume
                anomalies.append(
                    f"Unusually low volume: {volume_ratio:.2f}x median"
                )
    ...
    def _trigger_alerts(self, source: DataSourceType, metrics: QualityMetrics):
        ...
        alert_msg = (
            f"Data quality alert for {source.value}: "
            f"Grade={metrics.grade.value}, Score={metrics.overall_score:.2f}"
        )
    ...
    def get_source_reliability_report(self) -> Dict[str, Any]:
        ...
        for source in DataSourceType:
            ...
            report[source.value] = {
                ...
                if source in self.reliability_tracker.last_update else None,
                "grade": self._determine_grade(reliability).value
            }
        
        return report
    
    def get_symbol_quality_trend(
        self,
        symbol: str,
        hours: int = 24
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_known_good": (
                ...
                if symbol in self.last_known_good else None
            )
        }
...
class MockKiteClient:
    ...
    async def get_positions(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for position in self.positions:
            ...
            pnl_percent = (pnl / (position["average_price"] * abs(position["quantity"]))) * 100
    ...
    def _update_positions(self, order: Dict[str, Any]):
        ...
        price = order.get("price", self.market_data.get(symbol, {"last_price": 2500})["last_price"])
        ...
        if order["transaction_type"] == "BUY":
            if existing_position:
                ...
                total_value = (existing_position["average_price"] * existing_position["quantity"]) + (price * quantity)

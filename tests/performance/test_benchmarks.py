...
class TestPerformanceBenchmarks:
    ...
    @pytest.mark.asyncio
    async def test_agent_analysis_performance(
        self, mock_crew_manager, benchmark_metrics
    ):
        ...
        assert (
            summary["throughput"]["mean"] > 2.5
        )  # Should process > 2.5 symbols/second
    ...
    async def test_concurrent_order_execution(
        self, mock_kite_client, benchmark_metrics
    ):
        ...
        async def place_order(symbol: str, i: int):
            ...
            order = await mock_kite_client.place_order(
                symbol=symbol, quantity=100, order_type="BUY", price_type="MARKET"
            )
            return order
    ...
    async def test_websocket_message_processing(self, benchmark_metrics):
        ...
        for i in range(num_messages):
            ...
            ws_manager._prepare_message(message)
    ...
    def test_generate_performance_report(self, tmp_path):
        ...
        results = {
            ...
            "Data Pipeline": {"throughput": "250 ticks/sec", "latency": "4ms"},
            "Order Execution": {"throughput": "50 orders/sec", "success_rate": "99.9%"},
...
class TestAutomatedTradingPerformance:
    """Performance tests for automated trading system"""

    @pytest.fixture
    def trading_service(self):
        """Create trading service with mocks"""
        service = AutomatedTradingService()
        service.kite_client = MockKiteClient()
        service.crew_manager = MockCrewManager()
        return service

    @pytest.mark.asyncio
    async def test_trading_analysis_performance(self, trading_service):
        """Test performance of trading analysis"""
        ...
        for _ in range(10):
            ...
            def analysis_task():
                return asyncio.run(trading_service.analyze_and_trade())

            mem_usage = memory_usage(analysis_task, interval=0.1)

            execution_time = time.time() - start_time

            metrics.add_execution_time(execution_time)
            metrics.add_memory_usage(max(mem_usage))
            metrics.add_throughput(1 / execution_time)
        ...
        print(f"Memory usage: {summary['memory_usage']['mean']:.2f} MB")
        print(f"Throughput: {summary['throughput']['mean']:.2f} analyses/sec")

        # Performance assertions
        assert summary["execution_time"]["mean"] < 5.0  # Less than 5 seconds
        assert summary["memory_usage"]["mean"] < 200  # Less than 200MB
        assert summary["throughput"]["mean"] > 0.2  # At least 0.2 analyses/sec
    ...
    async def test_concurrent_trading_operations(self, trading_service):
        ...
        async def trading_operation():
            """Single trading operation"""
            start_time = time.time()
            await trading_service.analyze_and_trade()
            return time.time() - start_time

        # Run concurrent operations
        tasks = [trading_operation() for _ in range(5)]
        ...
        print(f"Concurrent throughput: {len(tasks) / max_time:.2f} ops/sec")

        # Performance assertions
        assert avg_time < 10.0  # Average less than 10 seconds
        assert max_time < 15.0  # Maximum less than 15 seconds

    def test_memory_leak_detection(self, trading_service):
        """Test for memory leaks in trading operations"""
        initial_memory = memory_usage()[0]

        # Run many operations
        for _ in range(50):
            asyncio.run(trading_service.analyze_and_trade())
        ...
        # Memory leak assertion (should not increase by more than 50MB)
        assert (
            memory_increase < 50

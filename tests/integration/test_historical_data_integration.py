...
class TestHistoricalDataIntegration:
    ...
    @pytest.mark.asyncio
    async def test_backtest_trading_strategy(self, crew_manager, historical_data, mock_kite_client):
    ...
    async def test_market_regime_detection(self, historical_data):
        ...
        assert all(regime in ['bullish', 'bearish', 'neutral'] for regime in regime_counts.index)
    ...
    async def test_signal_generation_accuracy(self, crew_manager, historical_data):
        ...
        for i in range(20, len(historical_data) - 1):
            ...
            if i % 10 == 0:  # Generate signal every 10 days
                ...
                actual_movement = historical_data[i + 1]["close"] - historical_data[i]["close"]
        ...
        if len(signals) > 0:
            ...
            weighted_accuracy = sum(s["confidence"] for s in signals if s["correct"]) / sum(s["confidence"] for s in signals)

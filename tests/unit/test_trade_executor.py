...
from app.schemas.trading import TradeSignal, OrderType, ProductType
...
class TestTradeExecutorAgent:
    ...
    async def test_handle_partial_fills(self, agent, mock_kite_client):
        ...
        with patch('agents.trade_executor.agent.kite_client', mock_kite_client):
            ...
            agent.order_manager._execute = AsyncMock(return_value={
                "action": "modify_order",
                "new_price": 2501,
    ...
    async def test_trailing_stop_update(self, agent, mock_kite_client):
        ...
        with patch('agents.trade_executor.agent.kite_client', mock_kite_client):
            ...
            result = await agent.update_trailing_stop(
                position,
    ...
    @pytest.mark.asyncio
    async def test_smart_order_routing(self, agent, sample_trade_signal, mock_kite_client):
    ...
    @pytest.mark.asyncio
    async def test_order_retry_mechanism(self, agent, sample_trade_signal, mock_kite_client):

...
class IndicatorCalculator:
    ...
    def calculate_rsi(
        self, data: pd.DataFrame, period: int = 14, price_column: str = "close"
    ) -> dict[str, Any]:
        """
    ...
    def calculate_bollinger_bands(
        ...
        try:
            ...
            position = self._get_band_position(
            ...
            return {
                ...
                "band_width": (
                    (upper[-1] - lower[-1]) if not np.isnan(upper[-1]) else None
                ),
                "position": position,
        ...
        try:
            ...
            if 50 in periods and 200 in periods:
                cross = self._check_ma_cross(
                    sma_results["SMA_50"]["values"], sma_results["SMA_200"]["values"]
                )
    ...
    def _calculate_rsi_pandas(self, prices: np.ndarray, period: int) -> np.ndarray:
        ...
        if avg_loss == 0:
            rsi[period] = 100.0  # All gains, no losses
        else:
        ...
        for i in range(period + 1, len(prices)):
            ...
            if avg_loss == 0:
                rsi[i] = 100.0  # All gains, no losses
            else:
                rs = avg_gain / avg_loss
    ...
    def _calculate_macd_pandas(
        ...
        Uses proper EMA calculation with validation and error handling.
        """
        if len(prices) < max(fast, slow, signal):
            min_required = max(fast, slow, signal)
            logger.warning(
                f"Insufficient data for MACD calculation. Need {min_required}, got {len(prices)}"
            )
            nan_array = np.full(len(prices), np.nan)
            return nan_array, nan_array, nan_array

        if fast >= slow:
            logger.error(f"Fast period ({fast}) must be less than slow period ({slow})")
            nan_array = np.full(len(prices), np.nan)
            return nan_array, nan_array, nan_array

        prices_series = pd.Series(prices)

        # Calculate EMAs with proper alpha values
        # Alpha = 2 / (period + 1) for EMA
    ...
    def _check_crossover(self, line1: np.ndarray, line2: np.ndarray) -> str | None:
    ...
    def _check_ma_cross(self, fast_ma: np.ndarray, slow_ma: np.ndarray) -> str | None:

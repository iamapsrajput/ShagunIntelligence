...
class RollingDataManager:
    ...
    def __init__(self, max_window_size: int = 500):
        ...
        logger.info(f"RollingDataManager initialized with window size: {max_window_size}")
    ...
    def get_cached_indicator(
        ...
        if cache_key in self.indicator_cache and indicator in self.indicator_cache[cache_key]:
            ...
            try:
                ...
                for symbol in list(self.data_store.keys()):
                    ...
                    for key in list(self.data_store[symbol].keys()):
                        timeframe = key.replace(f"{symbol}_", "")
                        last_update = self.last_update[symbol].get(key)
                        
                        if last_update and (current_time - last_update) > timedelta(hours=1):
                            timeframes_to_remove.append(key)
                            stats["data_points_removed"] += len(self.data_store[symbol][key])

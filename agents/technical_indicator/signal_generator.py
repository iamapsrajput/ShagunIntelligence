...
class SignalGenerator:
    ...
    def __init__(self):
        ...
        self.signal_weights = {"RSI": 0.25, "MACD": 0.30, "BB": 0.20, "MA_CROSS": 0.25}
    ...
    def generate_signals(
        ...
        try:
            ...
            if volume_confirmation:
                combined_signal[
                    "confidence"
                ] *= 1.1  # 10% boost for volume confirmation
    ...
    def _generate_rsi_signal(self, rsi_data: dict[str, Any]) -> dict[str, Any]:
        """
        Generate signal from RSI indicator with improved accuracy.

        Uses multiple RSI levels and trend analysis for better signals.
        """
        ...
        # Check RSI values history for divergence and trend
        rsi_values = rsi_data.get("values", [])
        divergence = (
            self._check_rsi_divergence(rsi_values) if len(rsi_values) > 20 else None
        )

        # Calculate RSI trend (last 5 periods)
        rsi_trend = None
        if len(rsi_values) >= 5:
            recent_rsi = rsi_values[-5:]
            if all(
                recent_rsi[i] < recent_rsi[i + 1] for i in range(len(recent_rsi) - 1)
            ):
                rsi_trend = "rising"
            elif all(
                recent_rsi[i] > recent_rsi[i + 1] for i in range(len(recent_rsi) - 1)
        ...
        # Enhanced RSI signal logic with multiple levels
        if current_rsi <= 20:
        ...
        if divergence == "bullish" and signal in [
            SignalType.BUY,
            SignalType.STRONG_BUY,
        ]:
            confidence = min(confidence + 0.15, 1.0)
            reasons.append("Bullish divergence detected")
        elif divergence == "bearish" and signal in [
            SignalType.SELL,
        ...
        if rsi_trend == "rising" and signal in [SignalType.BUY, SignalType.STRONG_BUY]:
            confidence = min(confidence + 0.1, 1.0)
            reasons.append("RSI trending upward")
        elif rsi_trend == "falling" and signal in [
            SignalType.SELL,
            SignalType.STRONG_SELL,
        ]:
            confidence = min(confidence + 0.1, 1.0)
            reasons.append("RSI trending downward")
    ...
    def _generate_ma_signal(self, indicators: dict[str, Any]) -> dict[str, Any] | None:
    ...
    def _check_rsi_divergence(self, rsi_values: np.ndarray) -> str | None:

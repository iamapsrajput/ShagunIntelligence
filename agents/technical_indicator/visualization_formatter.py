...
class VisualizationFormatter:
        ...
        try:
            ...
            for indicator_name, indicator_data in indicators.items():
                if indicator_name == "RSI":
                    formatted_data["indicators"]["rsi"] = self._format_rsi(indicator_data)
                elif indicator_name == "MACD":
                    formatted_data["indicators"]["macd"] = self._format_macd(indicator_data)
                elif indicator_name == "BB":
                    formatted_data["indicators"]["bollinger_bands"] = self._format_bollinger_bands(
    ...
    def _format_moving_averages(
        ...
        for key, values in ma_data.items():
            if key.startswith(ma_type):
                ...
                for timestamp, value in zip(timestamps, data_values):
                    if not np.isnan(value):
    ...
    def _format_signals(
        ...
        for signal_name, signal_data in individual_signals.items():
            annotation["indicators"].append({
                ...
                "signal": signal_data.get("signal", "").value if hasattr(signal_data.get("signal", ""), "value") else str(signal_data.get("signal", "")),
    ...
    def _generate_metadata(
        ...
        return {
            ...
            "start_time": price_data.index[0].isoformat() if isinstance(price_data.index[0], datetime) else str(price_data.index[0]),
            "end_time": price_data.index[-1].isoformat() if isinstance(price_data.index[-1], datetime) else str(price_data.index[-1]),

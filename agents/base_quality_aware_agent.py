...
from backend.data_sources.data_quality_validator import DataQualityValidator
...
class BaseQualityAwareAgent:
    ...
    async def get_quality_weighted_data(
    ...
    ) -> tuple[dict[str, Any] | None, float, DataQualityLevel]:
        """
        ...
        try:
            ...
            if hasattr(self.data_integration, "market_source_manager"):
                ...
                healthy_sources = [
                    name
                    for name, status in sources_status["sources"].items()
                ...
                for source_name in healthy_sources[:2]:
                    if source_name != data.get("data_source"):
                        try:
                            ref_quote = await self._get_reference_quote(
                                symbol, source_name
                            )
                            if ref_quote:
            ...
            quality_metrics = self.quality_validator.validate_data(
                market_data, DataSourceType.MARKET_DATA, reference_data
            )
    ...
    async def get_multi_source_consensus(
        self, symbol: str, min_sources: int = 2
        ...
        try:
            ...
            if hasattr(self.data_integration, "market_source_manager"):
                ...
                for source_name in sources_status["sources"]:
                    try:
                        quote = await self._get_source_specific_quote(
                            symbol, source_name
                        )
                        if quote:
    ...
    def adjust_position_size(
        self, base_size: float, quality_level: DataQualityLevel
    ) -> float:
    ...
    def get_confidence_score(
        self, quality_score: float, additional_factors: dict[str, float] = None
    ) -> float:
        """
    ...
    async def _get_reference_quote(
        self, symbol: str, source_name: str
    ) -> dict[str, Any] | None:
    ...
    def _calculate_consensus(self, quotes: dict[str, dict[str, Any]]) -> dict[str, Any]:
        ...
        # Get all prices
        prices = [
            q.get("current_price", 0) for q in quotes.values() if q.get("current_price")
        ]
        ...
        if best_quote:
            ...
            best_quote["_price_variance"] = (
                max(prices) - min(prices) if len(prices) > 1 else 0
            )

        return best_quote

    def _calculate_confidence(self, quotes: dict[str, dict[str, Any]]) -> float:

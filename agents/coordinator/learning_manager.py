...
class LearningManager:
    ...
    def __init__(self, learning_rate: float = 0.1):
        ...
        self.agent_contributions = defaultdict(lambda: {
    ...
    def record_decision_outcome(self, decision: Dict[str, Any], 
                              success: bool, outcome_data: Optional[Dict] = None) -> None:
        ...
        logger.info(f"Recorded outcome for {decision['symbol']}: "
                   f"{'Success' if success else 'Failure'}")
    
    def _update_strategy_performance(self, decision: Dict[str, Any],
                                   success: bool, outcome_data: Optional[Dict]) -> None:
        ...
        if strategy in self.strategy_performance:
            ...
            if outcome_data and "return" in outcome_data:
                ...
                perf["avg_return"] = (
                    (current_avg * (total_trades - 1) + new_return) / total_trades
                )
    ...
    def _update_agent_contributions(self, decision: Dict[str, Any], success: bool) -> None:
    ...
    def get_adaptation_recommendations(self, performance: Dict[str, Any],
                                     opportunities: List[Any],
                                     decisions: List[Dict]) -> List[Adaptation]:
    ...
    def _analyze_performance_patterns(self) -> Dict[str, Any]:
        ...
        failures = [e for e in recent_events if not e.outcome_data.get("success", False)]
        if failures:
    ...
    def _recommend_parameter_updates(self, performance: Dict[str, Any],
                                   analysis: Dict[str, Any]) -> List[Adaptation]:
        ...
        if performance.get("recent_volatility", 0) > performance.get("avg_volatility", 0) * 1.5:
            adaptations.append(Adaptation(
                adaptation_type=AdaptationType.PARAMETER_UPDATE,
    ...
    def _recommend_weight_adjustments(self, analysis: Dict[str, Any]) -> List[Adaptation]:
    ...
    def _recommend_strategy_changes(self, analysis: Dict[str, Any]) -> List[Adaptation]:
        ...
        if best_strategy:
            adaptations.append(Adaptation(
                ...
                reason=f"Switch to best performing strategy",
                confidence=0.7,
        ...
        for agent, contrib in self.agent_contributions.items():
            if contrib["contribution_score"] < 0.3:  # Poor performance
                adaptations.append(Adaptation(
                    ...
                    reason=f"Reduce influence of underperforming agent",
                    confidence=0.8,
    ...
    def _calculate_recent_performance(self) -> Dict[str, Any]:
        ...
        successes = sum(1 for e in recent_events if e.outcome_data.get("success", False))

...
class PerformanceMonitor:
    ...
    def record_trade_entry(self, trade_id: str, symbol: str, side: str,
        ...
        logger.info(f"Recorded trade entry: {trade_id} - {symbol} {side} "
                   f"{quantity} @ {entry_price}")
    
    def record_trade_exit(self, trade_id: str, exit_price: float,
                         commission: float = 0.0) -> Optional[TradeMetrics]:
    ...
    def _update_metrics_on_trade_close(self, trade: TradeMetrics, commission: float) -> None:
    ...
    def _update_aggregate_metrics(self) -> None:
        ...
        self.current_metrics["win_rate"] = (len(wins) / total_trades * 100) if total_trades > 0 else 0
        
        # Average win/loss
        self.current_metrics["avg_win"] = np.mean([t.pnl for t in wins]) if wins else 0
        self.current_metrics["avg_loss"] = np.mean([t.pnl for t in losses]) if losses else 0
        
        # Profit factor
        total_wins = sum(t.pnl for t in wins) if wins else 0
        total_losses = abs(sum(t.pnl for t in losses)) if losses else 1
        self.current_metrics["profit_factor"] = total_wins / total_losses if total_losses > 0 else 0
    ...
    def _calculate_period_returns(self) -> None:
        ...
        weekly_trades = [
            ...
            if t.exit_time and t.exit_time >= week_start
        ]
        ...
        monthly_trades = [
            t for t in self.closed_trades 
            if t.exit_time and t.exit_time >= month_start
        ]
    ...
    def _calculate_sharpe_ratio(self) -> None:
        ...
        returns = np.array([r for r in self.daily_returns])
    ...
    def _update_risk_metrics(self) -> None:
        ...
        if len(returns) > 0:
            ...
            self.risk_metrics["conditional_var"] = np.mean(beyond_var) if len(beyond_var) > 0 else var_threshold
        ...
        if self.active_trades:
            ...
            for trade_id, trade in self.active_trades.items():
                ...
                self.risk_metrics["position_concentration"][trade.symbol] = concentration
    ...
    def get_current_performance(self) -> Dict[str, Any]:
        ...
        return {
            **self.current_metrics,
            ...
            "current_streak_type": "wins" if self.current_metrics["consecutive_wins"] > 0 else "losses"
        }
    ...
    def get_trade_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_holding_period_hours": np.mean(holding_periods) if holding_periods else 0,
            "median_holding_period_hours": np.median(holding_periods) if holding_periods else 0,
            "best_trade": {
    ...
    def get_time_based_analysis(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "best_hour": max(hourly_stats.items(), 
                           key=lambda x: x[1]["avg_pnl"])[0] if hourly_stats else None,
            "worst_hour": min(hourly_stats.items(), 
                            key=lambda x: x[1]["avg_pnl"])[0] if hourly_stats else None
        }

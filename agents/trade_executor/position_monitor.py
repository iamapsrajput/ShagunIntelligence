...
class PositionMonitor:
    ...
    def add_position(self, order_id: str, signal: Any) -> None:
        ...
        try:
            ...
            if order_details and order_details.get("filled_quantity", 0) > 0:
                ...
                logger.info(f"Added position to monitor: {position.symbol} - {position.side}")
        ...
        try:
            ...
            if current_price > 0:
                ...
                if abs(position.pnl_percent) > 1.0:
                    logger.info(f"Position {position.symbol}: P&L {position.pnl_percent:.2f}%")
        ...
        return [
            {
                ...
                "duration": (datetime.now() - pos.entry_time).total_seconds() / 60,  # minutes
    ...
    def get_position_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "win_rate": (
                ...
                len(self.closed_positions) * 100
                if self.closed_positions else 0
            ),
            "average_win": (
                sum(p.pnl for p in self.closed_positions if p.pnl > 0) /
                len([p for p in self.closed_positions if p.pnl > 0])
                if [p for p in self.closed_positions if p.pnl > 0] else 0
            ),
            "average_loss": (
                sum(p.pnl for p in self.closed_positions if p.pnl < 0) /
                len([p for p in self.closed_positions if p.pnl < 0])
    ...
    def update_stop_loss(self, order_id: str, new_stop_loss: float) -> bool:
        ...
        if order_id in self.positions:
            ...
            logger.info(f"Updated stop loss for {self.positions[order_id].symbol} to {new_stop_loss}")
            return True
        return False
    ...
    def update_target(self, order_id: str, new_target: float) -> bool:
        ...
        if order_id in self.positions:
            ...
            logger.info(f"Updated target for {self.positions[order_id].symbol} to {new_target}")
            return True
        return False
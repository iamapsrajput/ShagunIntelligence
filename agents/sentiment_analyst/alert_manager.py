...
class AlertManager:
    ...
    async def create_sentiment_alert(
        ...
        try:
            ...
            logger.info(f"Created {alert_type.value} alert for {symbol}: change={change:.2f}")
            return alert
    ...
    async def create_volume_alert(
        ...
        try:
            ...
            alert = {
                ...
                'symbol': symbol,
                'timestamp': datetime.now(),
                'priority': AlertPriority.HIGH.value if volume_ratio > 3 else AlertPriority.MEDIUM.value,
                'data': {
                    'volume_ratio': volume_ratio,
                    'sentiment_score': sentiment_score,
                    'sentiment_context': self._interpret_volume_sentiment(volume_ratio, sentiment_score)
                },
    ...
    def _is_in_cooldown(self, identifier: str, alert_type: AlertType) -> bool:
        ...
        recent_alerts = [
            alert for alert in self.alert_history[identifier]
            if alert['type'] == alert_type.value and alert['timestamp'] > cutoff_time
        ]
        
        return len(recent_alerts) > 0

    def _calculate_priority(self, change_magnitude: float, confidence: float) -> AlertPriority:
    ...
    def _generate_alert_message(
        ...
        if alert_type == AlertType.SENTIMENT_SPIKE:
            return (f"🚀 Positive sentiment spike for {symbol}: "
                   f"+{abs(change)*100:.1f}% change to {current_score:.2f}")
        elif alert_type == AlertType.SENTIMENT_CRASH:
            return (f"📉 Negative sentiment crash for {symbol}: "
                   f"-{abs(change)*100:.1f}% change to {current_score:.2f}")

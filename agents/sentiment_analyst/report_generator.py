...
class ReportGenerator:
        ...
        try:
            ...
            for symbol in current_analyses:
                if symbol in previous_analyses:
                    current_score = current_analyses[symbol]["sentiment_scores"][
                        "overall_score"
                    ]
                    previous_score = previous_analyses[symbol]["sentiment_scores"][
                        "overall_score"
                    ]
            ...
            significant_changes = sorted(
                changes.items(), key=lambda x: abs(x[1]["change"]), reverse=True
            )[:10]
    ...
    def _aggregate_symbol_metrics(
        self, symbol_analyses: dict[str, dict[str, Any]]
    ) -> dict[str, Any]:
        ...
        return {
            "average_sentiment": (
                statistics.mean(sentiment_scores) if sentiment_scores else 0
            ),
            "sentiment_std_dev": (
    ...
    def _identify_top_movers(
        ...
        high_confidence = sorted(
            symbol_data, key=lambda x: x["confidence"], reverse=True
        )
    ...
    def _analyze_trends(
                ...
                improving.append(
                    {
                        "symbol": symbol,
                        "change": change,
                        "current_sentiment": analysis["sentiment_scores"][
                            "overall_score"
                        ],
                    }
                )
        ...
        return {
            "improving_sentiment": sorted(
                improving, key=lambda x: x["change"], reverse=True
            ),
    ...
    def _generate_executive_summary(
        ...
        total_coverage = aggregated_metrics.get(
            "total_news_articles", 0
        ) + aggregated_metrics.get("total_social_posts", 0)
            ...
            formatted.append(
                {
                    ...
                    "coverage": {
                        ...
                        "social_posts": (
                            analysis.get("social_analysis", {}).get("total_posts", 0)
                            if analysis.get("social_analysis")
                            else 0
                        ),
                    },
                    "key_stories": [
    ...
    def _summarize_alerts(self, alerts: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        return {
            ...
            "most_alerted_symbols": sorted(
                alert_symbols.items(), key=lambda x: x[1], reverse=True
            )[:5],
    ...
    def _assess_market_risk(
        self, aggregated_metrics: dict[str, Any], market_analysis: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _summarize_changes(self, changes: dict[str, dict[str, Any]]) -> dict[str, Any]:
        ...
        avg_change = (
            statistics.mean([c["change"] for c in changes.values()]) if changes else 0
        )

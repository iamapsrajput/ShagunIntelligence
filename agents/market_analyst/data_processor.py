...
class RealTimeDataProcessor:
    ...
    def __init__(self, kite_service: KiteConnectService, max_history: int = 1000):
        ...
        # Data storage
        self.tick_data: dict[int, deque] = defaultdict(
            lambda: deque(maxlen=max_history)
        )
        self.current_ticks: dict[int, TickData] = {}
        self.candle_data: dict[str, dict[str, deque]] = defaultdict(
    ...
    async def initialize(self, symbols: list[str]) -> bool:
        ...
        try:
            logger.info(
                f"Initializing real-time data processor for {len(symbols)} symbols"
            )
            ...
            for symbol in symbols:
                for instrument in instruments:
                    if (
                        ...
                        self.candle_builders["1min"][token] = CandleBuilder(
                            symbol, token, 60
                        )  # 1 minute
                        self.candle_builders["5min"][token] = CandleBuilder(
                            symbol, token, 300
                        )  # 5 minutes
                        self.candle_builders["15min"][token] = CandleBuilder(
                            symbol, token, 900
                        )  # 15 minutes
                        break
            ...
            if (
                hasattr(self.kite_service, "websocket_client")
                and self.kite_service.websocket_client
            ):
                self.kite_service.websocket_client.set_on_tick_callback(
                    self._on_tick_received
                )

            logger.info(
                f"Data processor initialized for {self.stats['symbols_count']} symbols"
            )
            return True
    ...
    def _process_tick(self, tick: dict[str, Any]):
        ...
        try:
            ...
            tick_data = TickData(
                ...
                change_percent=(
                    tick.get("change", 0) / tick.get("last_price", 1) * 100
            ...
            logger.error(
                f"Error processing tick for {tick.get('instrument_token')}: {str(e)}"
            )
    ...
    def get_candle_data(
        ...
        try:
            with self._data_lock:
                if (
                    timeframe in self.candle_data
                    and symbol in self.candle_data[timeframe]
                ):
    ...
    def get_processing_stats(self) -> dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "memory_usage": {
                "tick_data_size": sum(
                    len(deque_data) for deque_data in self.tick_data.values()
                ),
...
class CandleBuilder:
    ...
    def __init__(self, symbol: str, instrument_token: int, interval_seconds: int):
        ...
        self.current_candle: CandleData | None = None
        self.current_interval_start: datetime | None = None

    def add_tick(self, tick: TickData) -> CandleData | None:
        ...
        try:
            ...
            if self.current_candle:
                self.current_candle.high = max(
                    self.current_candle.high, tick.last_price
                )
    ...
    def _get_interval_start(self, timestamp: datetime) -> datetime:
        ...
        return datetime.combine(timestamp.date(), datetime.min.time()) + timedelta(
            seconds=interval_start_seconds
        )

    def get_current_candle(self) -> CandleData | None:

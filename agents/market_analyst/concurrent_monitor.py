...
from .watchlist_manager import WatchlistManager, TradingOpportunity
...
class SymbolMonitor:
    ...
    async def _full_analysis(self) -> Dict[str, Any]:
        ...
        try:
            # Run all analyzers
            statistical_analysis = self.analyzers['statistical'].analyze_symbol(self.symbol)
            pattern_analysis = self.analyzers['pattern'].analyze_patterns(self.symbol)
            volume_signals = self.analyzers['volume'].generate_volume_signals(self.symbol)
            volume_anomalies = self.analyzers['volume'].anomaly_detector.detect_anomalies(self.symbol)
            
            return {
                'symbol': self.symbol,
                'statistical_analysis': statistical_analysis.to_dict() if statistical_analysis else None,
                'pattern_analysis': pattern_analysis,
                'volume_signals': [signal.to_dict() if hasattr(signal, 'to_dict') else signal for signal in volume_signals],
    ...
    async def _volume_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            anomalies = volume_analyzer.anomaly_detector.detect_anomalies(self.symbol, ['1min'])
...
class ConcurrentMonitor:
    ...
    async def start_monitoring(self):
        ...
        result_processor_task = asyncio.create_task(self._result_processor())
        ...
        while self.is_running:
            try:
                ...
                for symbol, monitor in self.monitors.items():
                    ...
                    for task_type in ['volume_check', 'quick_scan', 'pattern_check', 'full_analysis']:
        ...
        while self.is_running:
            try:
                ...
                if result.success and result.task_type == 'full_analysis':
                    logger.debug(f"Completed full analysis for {result.symbol} in {result.processing_time:.2f}s")
                elif not result.success:
                    logger.warning(f"Failed {result.task_type} for {result.symbol}: {result.error}")
    ...
    async def force_analysis(self, symbol: str, task_type: str = 'full_analysis') -> Optional[MonitoringResult]:

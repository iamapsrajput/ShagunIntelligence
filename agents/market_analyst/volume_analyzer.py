...
try:
    ...
    logger.warning(
        "TA-Lib not available. Using pandas implementations for volume indicators."
    )
...
class VolumeIndicators:
    ...
    def calculate_volume_indicators(df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if TALIB_AVAILABLE:
                ...
                df["obv"] = VolumeIndicators._calculate_obv_pandas(df)
                df["obv_sma"] = df["obv"].rolling(window=20).mean()
                df["ad_line"] = VolumeIndicators._calculate_ad_pandas(df)
                df["ad_sma"] = df["ad_line"].rolling(window=20).mean()
    ...
    def _calculate_klinger_oscillator(df: pd.DataFrame) -> pd.Series:
        ...
        try:
            ...
            dm = df["high"] - df["low"]
            cm = np.where(
                trend == trend.shift(1), dm + (dm.shift(1) if len(dm) > 1 else 0), dm
            )
            ...
            if TALIB_AVAILABLE:
                klinger = talib.EMA(vf.values, timeperiod=34) - talib.EMA(
                    vf.values, timeperiod=55
                )
            else:
                # Pandas EMA fallback
                ema34 = vf.ewm(span=34).mean()
                ema55 = vf.ewm(span=55).mean()
                klinger = ema34 - ema55
...
class VolumeAnomalyDetector:
    ...
    def detect_anomalies(
        ...
        for timeframe in timeframes:
            try:
                timeframe_anomalies = self._detect_timeframe_anomalies(
                    symbol, timeframe
                )
                anomalies.extend(timeframe_anomalies)
            except Exception as e:
                logger.error(
                    f"Error detecting anomalies for {symbol} on {timeframe}: {str(e)}"
                )
                continue

        return anomalies

    def _detect_timeframe_anomalies(
        ...
        try:
            ...
            if current_volume > avg_volume + (self.spike_threshold * volume_std):
                severity = min(
                    (current_volume - avg_volume) / (self.spike_threshold * volume_std),
                    1.0,
                )
                ...
                anomalies.append(
                    VolumeAnomaly(
                        ...
                        supporting_data={
                            "timeframe": timeframe,
                            "std_deviations": (current_volume - avg_volume)
                            / volume_std,
            ...
            # Detect volume droughts
                ...
                severity = 1.0 - (
                    current_volume / (avg_volume * self.drought_threshold)
                )
            ...
            if len(df) >= 50:
                ...
                if len(volume_features) > 0:
                    try:
                        ...
                        historical_features = volume_features[
                            :-5
                        ]  # Exclude recent data
                        ...
                        if any(outliers == -1):
                            ...
                            if outliers[most_anomalous_idx] == -1:
                                severity = min(
                                    abs(anomaly_scores[most_anomalous_idx]) / 0.5, 1.0
                                )
    ...
    def _extract_volume_features(self, df: pd.DataFrame) -> np.ndarray:
        ...
        try:
            ...
            for i in range(window_size, len(df)):
                ...
                # Volume ratios
                vol_ratio = (
                    window_data["volume"].iloc[-1] / vol_mean if vol_mean > 0 else 1
                )
                ...
                # OBV trend
                obv_change = (
                    (window_data["obv"].iloc[-1] - window_data["obv"].iloc[0])
                    / abs(window_data["obv"].iloc[0])
                    if window_data["obv"].iloc[0] != 0
                    else 0
                )
...
class VolumeProfileAnalyzer:
    ...
    def calculate_volume_profile(
        self, symbol: str, timeframe: str = "5min", lookback_periods: int = 100
    ) -> VolumeProfile | None:
        """Calculate volume profile for a symbol"""
        try:
            df = self.data_processor.get_candle_data(
            ...
            while va_volume < target_volume and (
                min(va_indices) > 0 or max(va_indices) < len(volume_at_price) - 1
            ):
                # Check adjacent levels
                left_volume = (
                    volume_at_price[min(va_indices) - 1] if min(va_indices) > 0 else 0
                )
                right_volume = (
                    volume_at_price[max(va_indices) + 1]
                    if max(va_indices) < len(volume_at_price) - 1
                    else 0
                )
            ...
            va_indices.sort()
            vah = (
                price_levels[max(va_indices)] + price_levels[max(va_indices) + 1]
            ) / 2
            val = (
                price_levels[min(va_indices)] + price_levels[min(va_indices) + 1]
            ) / 2
            ...
            return VolumeProfile(
                ...
                price_levels=price_levels[
                    :-1
                ].tolist(),  # Remove last level (upper bound)
...
class VolumeSignalGenerator:
    ...
    def generate_volume_signals(
        ...
        for timeframe in timeframes:
            try:
                ...
                logger.error(
                    f"Error generating volume signals for {symbol} on {timeframe}: {str(e)}"
                )
                continue

        return signals

    def _analyze_timeframe_volume(
        self, symbol: str, timeframe: str
    ) -> list[VolumeSignal]:
        ...
        try:
            ...
            df = VolumeIndicators.calculate_volume_indicators(df)
            latest = df.iloc[-1]
            df.iloc[-2] if len(df) > 1 else latest
    ...
    def _analyze_accumulation_distribution(
        ...
        try:
            ...
            if (signal_type == "accumulation" and ad_trend > 0) or (
                signal_type == "distribution" and ad_trend < 0
            ):
                signal_strength += 0.2
            ...
            if signal_strength > 0.3:
                return VolumeSignal(
                    ...
                    price_volume_relationship=(
                        "confirmative"
                        if abs(obv_trend * price_trend) > 0
                        else "divergent"
                    ),
                    timestamp=datetime.now(),
                    reasoning=reasoning,
    ...
    def _analyze_volume_breakout(
        ...
        try:
            volume_ratio = latest.get("volume_ratio", 1.0)
            price_change_percent = (
                (latest["close"] - df["close"].iloc[-2]) / df["close"].iloc[-2]
            ) * 100
    ...
    def _analyze_volume_climax(
        ...
        try:
            ...
            if volume_ratio > 2.5:
                ...
                price_momentum = (
                    talib.MOM(recent_prices.values, timeperiod=3)[-1]
                    if len(recent_prices) >= 4
                    else 0
                )
                price_range = (df["high"].tail(5).max() - df["low"].tail(5).min()) / df[
                    "close"
                ].tail(5).mean()

                if (
                    abs(price_momentum) < 0.5 and price_range > 0.03

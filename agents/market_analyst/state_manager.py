...
from .data_processor import TickData, CandleData
from .statistical_analyzer import TradingSignal, MarketStats
from .pattern_recognition import PatternMatch, BreakoutSignal
from .volume_analyzer import VolumeAnomaly, VolumeSignal
from .watchlist_manager import TradingOpportunity, WatchlistItem
...
class DatabaseManager:
    ...
    def _init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_opportunities_symbol_score ON trading_opportunities(symbol, score)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_anomalies_symbol_timestamp ON volume_anomalies(symbol, timestamp)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_patterns_symbol_timestamp ON pattern_matches(symbol, timestamp)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_stats_symbol_timestamp ON market_statistics(symbol, timestamp)')
    ...
    def get_recent_opportunities(self, symbol: str = None, hours: int = 24) -> List[Dict[str, Any]]:
    ...
    def cleanup_old_data(self, days_to_keep: int = 30):
        ...
        try:
            with self.get_connection() as conn:
                ...
                tables = ['trading_signals', 'trading_opportunities', 'volume_anomalies', 
                         'pattern_matches', 'market_statistics']
...
class SharedStateManager:
    ...
    def __init__(self, redis_url: str = "redis://localhost:6379", db_path: str = "data/market_analyst.db"):
    ...
    def store_volume_anomaly(self, anomaly: VolumeAnomaly):
        ...
        # Store in Redis
        self.redis_manager.set_analysis_result(anomaly.symbol, "volume_anomaly", anomaly)
        ...
        try:
            with self.db_manager.get_connection() as conn:
                ...
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    symbol, pattern.pattern_name, pattern.confidence, pattern.direction,
                    pattern.description, json.dumps(pattern.key_levels), timeframe, datetime.now()
                ))
                conn.commit()
        except Exception as e:
    ...
    def get_symbol_state(self, symbol: str) -> Dict[str, Any]:
        ...
        state = {
            ...
            'opportunities': self.redis_manager.get_state(f"opportunity:{symbol}") or [],
            'patterns': self.redis_manager.get_state(f"pattern:{symbol}") or [],
            'volume_anomaly': self.redis_manager.get_analysis_result(symbol, "volume_anomaly"),
            'timestamp': datetime.now()
        }
    ...
    def load_snapshot(self, filepath: str) -> Optional[StateSnapshot]:

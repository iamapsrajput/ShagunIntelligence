...
from .volume_analyzer import VolumeAnomaly, VolumeSignalGenerator
...
class OpportunityScorer:
    ...
    def score_opportunity(
        ...
        volume_signals: list[Any] | None = None,
        volume_anomalies: list[VolumeAnomaly] | None = None,
        ...
        try:
            ...
            scores["technical_analysis"] = self._score_technical_analysis(
                trading_signal
            )

            # Pattern recognition score
            scores["pattern_recognition"] = self._score_pattern_analysis(
                pattern_analysis
            )

            # Volume analysis score
            scores["volume_analysis"] = self._score_volume_analysis(
                volume_signals, volume_anomalies
            )
            ...
            expected_move = self._estimate_expected_move(
                trading_signal, pattern_analysis
            )
            time_horizon = self._determine_time_horizon(
                opportunity_type, pattern_analysis
            )

            # Determine alert level
            alert_level = self._determine_alert_level(
                final_score, confidence, opportunity_type
            )
            ...
            target_price, stop_loss = self._calculate_levels(
                trading_signal, entry_price, expected_move
            )
            ...
            signals = self._compile_signals(
                trading_signal, pattern_analysis, volume_signals
            )
            reasoning = self._compile_reasoning(
                scores, opportunity_type, trading_signal
            )
    ...
    def _determine_opportunity_type(
        self, trading_signal, pattern_analysis, volume_signals, volume_anomalies
    ) -> OpportunityType:
        ...
        if trading_signal:
            if trading_signal.signal_type in ["BUY", "SELL"]:
                ...
                if any(
                    ...
                    for word in ["reversal", "oversold", "overbought", "divergence"]
                ):
                    return OpportunityType.REVERSAL
                elif any(
                    word in reasoning_text for word in ["momentum", "trend", "breakout"]
                ):
    ...
    def _score_technical_analysis(
        self, trading_signal: TradingSignal | None
    ) -> float | None:
    ...
    def _score_pattern_analysis(
        self, pattern_analysis: dict[str, Any] | None
    ) -> float | None:
        ...
        for _timeframe, patterns in pattern_analysis.get("chart_patterns", {}).items():
            if patterns:
                for pattern in patterns:
                    confidence = (
                        pattern.confidence if hasattr(pattern, "confidence") else 0.5
                    )
                    direction_bonus = (
                        10 if pattern.direction in ["bullish", "bearish"] else 0
                    )
        ...
        if candlestick_patterns:
            for _timeframe, patterns in candlestick_patterns.items():
                score += len(patterns) * 5  # 5 points per pattern

        return min(score, 100)

    def _score_volume_analysis(
        self,
        volume_signals: list | None,
        volume_anomalies: list[VolumeAnomaly] | None,
    ) -> float | None:
        ...
        if volume_signals:
            for signal in volume_signals:
                signal_strength = (
                    signal.strength if hasattr(signal, "strength") else 0.5
                )
                score += signal_strength * 25

                # Bonus for specific signal types
                signal_type = (
                    signal.signal_type if hasattr(signal, "signal_type") else ""
                )
    ...
    def _score_momentum(
        self, trading_signal: TradingSignal | None, market_data: dict | None
    ) -> float | None:
    ...
    def _score_risk_reward(self, trading_signal: TradingSignal | None) -> float | None:
    ...
    def _score_market_conditions(self, market_data: dict | None) -> float | None:
    ...
    def _estimate_expected_move(
        self, trading_signal: TradingSignal | None, pattern_analysis: dict | None
    ) -> float:
    ...
    def _determine_alert_level(
        self, score: float, confidence: float, opportunity_type: OpportunityType
    ) -> AlertLevel:
    ...
    def _calculate_levels(
        ...
        if trading_signal and trading_signal.signal_type == "BUY":
            ...
            stop_loss = entry_price * (
                1 - expected_move / 200
            )  # Half the expected move for stop
    ...
    def _compile_signals(
        self, trading_signal, pattern_analysis, volume_signals
    ) -> list[dict[str, Any]]:
                ...
                signals.append(
                    {
                        ...
                        "strength": (
                            signal.strength if hasattr(signal, "strength") else 0
                        ),
                    }
                )
    ...
    def _compile_reasoning(
        ...
        for component, score in scores.items():
            if score is not None and score > 60:
                reasoning.append(
                    f"Strong {component.replace('_', ' ')}: {score:.1f}/100"
                )
...
class WatchlistManager:
    ...
    def add_symbol(
        self, symbol: str, watchlist_name: str = "default", priority: int = 3, **kwargs
    ) -> bool:
    ...
    async def scan_opportunities(
        self, watchlist_name: str = "default"
    ...
    def get_top_opportunities(
        self, watchlist_name: str = "default", count: int = 10
    ) -> list[TradingOpportunity]:
    ...
    def get_opportunities_by_type(
        self, opportunity_type: OpportunityType, watchlist_name: str = "default"
    ) -> list[TradingOpportunity]:
        """Get opportunities filtered by type"""
        opportunities = self.opportunities.get(watchlist_name, [])
        return [
            opp for opp in opportunities if opp.opportunity_type == opportunity_type
        ]
    ...
    def update_symbol_settings(
        self, symbol: str, watchlist_name: str = "default", **kwargs
    ) -> bool:
        ...
        return {
            ...
            "high_priority_opportunities": len(
                [
                    ...
                    if opp.alert_level in [AlertLevel.HIGH, AlertLevel.CRITICAL]
                ]
            ),
            "average_score": (
                np.mean([opp.score for opp in opportunities]) if opportunities else 0
            ),
            ...
            for name, items_data in data.items():
                ...
                for item_data in items_data:
                    item = WatchlistItem(
                        ...
                        last_analysis=(
                            datetime.fromisoformat(item_data["last_analysis"])
                            if item_data.get("last_analysis")
                            else None
                        ),
                    )
                    items.append(item)

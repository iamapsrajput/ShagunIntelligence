...
try:
    ...
    logger.warning(
        "TA-Lib not available. Using pandas implementations for technical indicators."
    )
...
class TradingSignal:
    ...
    risk_reward: tuple[float, float] | None = None  # (risk, reward)
...
class TechnicalIndicators:
    ...
    def calculate_all_indicators(df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df["bb_position"] = (close_prices - df["bb_lower"]) / (
            ...
            df["pivot_high"] = (
                df["high"].rolling(window=5, center=True).max() == df["high"]
            )
            df["pivot_low"] = (
                df["low"].rolling(window=5, center=True).min() == df["low"]
            )
...
class StatisticalAnalyzer:
    ...
    def _analyze_timeframe(self, symbol: str, timeframe: str) -> TradingSignal | None:
    ...
    def _analyze_volatility(
        ...
        try:
            ...
            if len(df) > 20:
                ...
                volatility_ratio = (
                    recent_volatility / avg_volatility if avg_volatility > 0 else 1
                )
        ...
        try:
            ...
            for signal in signals:
                ...
                signal_score = (
                    ...
                    else -signal.strength if signal.signal_type == "SELL" else 0
                )
    ...
    def _calculate_confidence(
        self, df: pd.DataFrame, signals: dict[str, tuple]
    ) -> float:
        ...
        try:
            ...
            # Signal consensus
            valid_signals = [
                score for score, _ in signals.values() if score is not None
            ]
            ...
            atr_percent = (
                df["atr_percent"].iloc[-1] if "atr_percent" in df.columns else 2.0
            )
    ...
    def _calculate_risk_reward(
        self, df: pd.DataFrame, latest: pd.Series, signal_type: str

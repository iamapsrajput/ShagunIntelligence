...
class PortfolioAnalyzer:
    ...
    def calculate_total_exposure(
        self, positions: list[dict[str, Any]], capital: float
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                "cash_percentage": (
                    (capital - gross_exposure) / capital if capital > 0 else 0
                ),
    ...
    def analyze_sector_concentration(
        ...
        try:
            ...
            overweight_sectors = [
                sector for sector, weight in sector_weights.items() if weight > 0.25
            ]
            ...
            return {
                ...
                "max_sector": (
                    max(sector_weights.items(), key=lambda x: x[1])[0]
                    if sector_weights
                    else None
                ),
        ...
        try:
            ...
            for position in positions:
                ...
                position_weights.append(
                    {"symbol": position["symbol"], "value": value, "weight": weight}
                )
            ...
            return {
                ...
                "max_position": (
                    position_weights[0]["symbol"] if position_weights else None
                ),
    ...
    def _generate_sector_recommendations(
        self, sector_weights: dict[str, float], risk_level: str
    ) -> list[str]:
        ...
        if sector_weights:
            ...
            if underweight:
                recommendations.append(
                    f"Consider exposure to: {', '.join(underweight[:3])}"
                )

        return recommendations

    def _generate_correlation_recommendations(
        self, high_correlations: list[dict[str, Any]], risk_level: str
    ) -> list[str]:
    ...
    def _generate_position_recommendations(
        self, max_weight: float, top_5_weight: float
    ) -> list[str]:
        ...
        if top_5_weight > 0.6:
            recommendations.append(
                "Top 5 positions exceed 60% - improve diversification"
            )
    ...
    def _identify_key_risks(
        ...
        if sector["concentration_risk"] in ["high", "critical"]:
            risks.append(
                f"High sector concentration ({sector['max_sector_weight']:.1%})"
            )

        if position["max_position_weight"] > 0.15:
            risks.append(
                f"Large position concentration ({position['max_position_weight']:.1%})"
            )

        if correlation["max_correlation"] > 0.8:
            risks.append(
                f"High correlation risk ({correlation['max_correlation']:.2f})"
            )
    ...
    def _generate_overall_recommendations(
        ...
        if exposure["percentage"] > 0.8:
            all_recommendations.append(
                "Reduce overall exposure to maintain cash buffer"
            )

...
from ..base_quality_aware_agent import DataQualityLevel
...
class EnhancedRiskManagerAgent(Agent):
    ...
    # Define additional fields for risk management
    capital: float = Field(default=100000, description="Total trading capital")
    max_risk_per_trade: float = Field(
        default=0.02, description="Maximum risk per trade"
    )
    max_portfolio_risk: float = Field(
        default=0.06, description="Maximum portfolio risk"
    )
    max_correlation: float = Field(
        default=0.7, description="Maximum allowed correlation"
    )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        # Initialize Agent with required fields and risk parameters
        super().__init__(
            role="Enhanced Risk Manager",
            goal="Manage trading risk with advanced quality-aware algorithms",
            backstory="Expert risk manager with deep understanding of market dynamics and data quality",
            llm=llm,
            capital=capital,
    ...
    def model_post_init(self, __context):
        ...
        object.__setattr__(self, "_data_integration", get_data_source_integration())
        object.__setattr__(self, "_quality_validator", DataQualityValidator())
        object.__setattr__(
            self,
            "_quality_thresholds",
            {
        ...
        object.__setattr__(
            ...
            {
                QualityGrade.EXCELLENT: 1.0,  # Normal risk limits
                QualityGrade.GOOD: 0.8,  # 80% of normal risk
                QualityGrade.ACCEPTABLE: 0.5,  # 50% of normal risk
                QualityGrade.POOR: 0.25,  # 25% of normal risk
                QualityGrade.FAILED: 0.0,  # No risk allowed
            },
        )

        # Initialize components
        object.__setattr__(
            self, "position_sizer", PositionSizer(self.capital, self.max_risk_per_trade)
        )
        object.__setattr__(self, "stop_loss_manager", StopLossManager())
        object.__setattr__(self, "portfolio_analyzer", PortfolioAnalyzer())
        object.__setattr__(self, "circuit_breaker", CircuitBreaker())
        object.__setattr__(self, "risk_metrics", RiskMetricsCalculator())

        # Risk tracking
        object.__setattr__(self, "active_positions", {})
        object.__setattr__(self, "risk_history", [])
        object.__setattr__(self, "blocked_trades", [])
    ...
    async def evaluate_trade_risk(
        ...
        try:
            ...
            self.max_risk_per_trade * quality_multiplier
    ...
    async def set_dynamic_stop_loss(
        ...
        try:
            ...
            if position_type == "long":
                ...
                optimal_stop = min(
                    trailing_stop, breakeven_stop or float("inf"), resistance * 1.02
                )
            ...
            return {
                ...
                "resistance_stop": (
                    resistance * 1.02 if position_type == "short" else None
                ),
    ...
    async def monitor_portfolio_risk(
        ...
        try:
            ...
            circuit_status = self.circuit_breaker.check_portfolio_circuit(
                total_exposure, portfolio_var, drawdown_info["current_drawdown"]
            )
    ...
    async def should_block_trade(
        ...
        try:
            ...
            if position_value > self.capital * 0.1:  # Max 10% in single position
                return True, "Position size exceeds 10% of capital limit"

            # Check total exposure
            current_exposure = sum(
                p["shares"] * p["current_price"] for p in current_portfolio
            )
    ...
    async def should_block_trade_quality_aware(
        ...
        try:
            ...
            if quality_level == DataQualityLevel.CRITICAL:
                return (
                    True,
                    f"Critical data quality ({data_quality_score:.1%}) - no trades allowed",
                )
            elif quality_level == DataQualityLevel.LOW:
                return (
                    True,
                    f"Low data quality ({data_quality_score:.1%}) - defensive mode only",
                )
            ...
            if quality_level == DataQualityLevel.MEDIUM:
                ...
                if position_value > self.capital * 0.05:  # Max 5% instead of 10%
                    return (
                        True,
                        "Position size exceeds 5% limit due to medium data quality",
                    )
    ...
    def _generate_recommendation(
        self, risk_score: float, risk_reward_ratio: float
    ) -> str:
    ...
    def _assess_portfolio_risk_level(
        self, exposure_pct: float, var_pct: float, max_correlation: float
    ) -> str:
    ...
    def _check_high_correlation(
        self, symbol: str, portfolio: list[dict[str, Any]]
    ) -> bool:
    ...
    async def calculate_quality_adjusted_position_size(
        ...
        try:
            ...
            adjusted_confidence = (
                base_confidence * quality_confidence_multiplier[quality_level]
            )

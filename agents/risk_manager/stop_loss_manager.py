...
class StopLossManager:
    ...
    def calculate_dynamic_stop(
        ...
        try:
            ...
            return entry_price * 0.98 if trade_direction == 'long' else entry_price * 1.02
        ...
        try:
            if position_type == 'long':
                if current_price <= entry_price:
                    ...
                    return self.calculate_dynamic_stop(entry_price, atr, entry_price * 0.97)
                ...
                if current_price >= entry_price:
                    # No trailing if price hasn't moved favorably
                    return self.calculate_dynamic_stop(entry_price, atr, entry_price * 1.03, 'short')
            ...
            return current_price * 0.98 if position_type == 'long' else current_price * 1.02
    ...
    def _cluster_levels(self, levels: List[float], tolerance: float = 0.002) -> List[Dict[str, Any]]:
    ...
    def _find_nearest_support(self, levels: List[Dict[str, Any]], current_price: float) -> float:
        """Find nearest support level below current price."""
        supports = [lvl['level'] for lvl in levels if lvl['level'] < current_price]
        return max(supports) if supports else 0

    def _find_nearest_resistance(self, levels: List[Dict[str, Any]], current_price: float) -> float:
    ...
    def update_settings(
        ...
        logger.info(f"Stop loss settings updated: ATR multiplier={self.atr_multiplier}, "
    ...
    def get_stop_statistics(self) -> Dict[str, Any]:
        ...
        for stop in self.stop_history:
            if stop['trade_direction'] == 'long':
                distance = (stop['entry_price'] - stop['stop_loss']) / stop['entry_price']
            else:
                distance = (stop['stop_loss'] - stop['entry_price']) / stop['entry_price']
        ...
        return {
            ...
            'methods_used': list(set(s.get('method', 'unknown') for s in self.stop_history))
        }
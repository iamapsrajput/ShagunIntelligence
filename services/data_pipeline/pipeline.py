...
class DataPipeline:
    ...
    def __init__(self, kite_client, config: PipelineConfig | None = None):
    ...
    def get_ohlc(self, symbol: str, interval: str = "1min") -> dict[str, Any] | None:
    ...
    async def add_symbols(self, symbols: list[str]) -> None:
        ...
        if total_symbols > self.config.max_symbols:
            logger.warning(
                f"Cannot add {len(new_symbols)} symbols. "
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        # Overall status
        if (
            health["checks"]["websocket"] == "disconnected"
            or health["checks"]["data_flow"] == "inactive"
            or health["checks"]["error_rate"] == "high"
        ):

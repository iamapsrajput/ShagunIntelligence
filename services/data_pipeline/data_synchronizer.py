...
class DataSynchronizer:
    ...
    def register_agent(self, agent_name: str, callback: Callable | None = None) -> None:
    ...
    async def get_shared_data(self, symbol: str) -> dict[str, Any] | None:
    ...
    async def _write_to_segment(self, segment: SharedDataSegment, data: bytes) -> None:
        ...
        if len(data) > segment.size:
            raise ValueError(
                f"Data size {len(data)} exceeds segment size {segment.size}"
            )
    ...
    async def _read_from_segment(self, segment: SharedDataSegment) -> bytes | None:
        ...
        try:
            ...
            # Read version
            int.from_bytes(segment.memory.buf[4:8], byteorder="big")
    ...
    async def get_agent_sync_queue(self, agent_name: str) -> asyncio.Queue | None:
        ...
        for agent_name, info in self.registered_agents.items():
            agent_stats[agent_name] = {
                ...
                "subscriptions": len(
                    [
                        s
                        for s, agents in self.agent_subscriptions.items()

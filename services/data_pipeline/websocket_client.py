...
class ZerodhaWebSocketClient:
    ...
    async def subscribe(
        self, tokens: list[int], mode: TickMode = TickMode.FULL
    ) -> None:
    ...
    async def unsubscribe(self, tokens: list[int]) -> None:
        ...
        message = {"type": MessageType.UNSUBSCRIBE.value, "tokens": tokens}
    ...
    async def set_mode(self, tokens: list[int], mode: TickMode) -> None:
        ...
        message = {"type": MessageType.MODE.value, "tokens": tokens, "mode": mode.value}
            ...
            if data.get("type") == "error":
                ...
                if self.on_error:
                    await self._safe_callback(
                        self.on_error, Exception(data.get("message", "Unknown error"))
                    )
            else:
    ...
    def _parse_binary_data(self, data: bytes) -> list[dict[str, Any]]:
        ...
        while offset < packet_length:
            try:
                ...
                tick["instrument_token"] = struct.unpack(
                ...
                if remaining >= 40:  # Full mode
                    # LTP (4 bytes)
                    tick["last_price"] = (
                        struct.unpack(">I", data[offset : offset + 4])[0] / 100
                    )
                    offset += 4

                    # Last quantity (4 bytes)
                    tick["last_quantity"] = struct.unpack(
                        ">I", data[offset : offset + 4]
                    )[0]
                    offset += 4
                    ...
                    tick["buy_quantity"] = struct.unpack(
                        ">I", data[offset : offset + 4]
                    )[0]
                    offset += 4

                    # Sell quantity (4 bytes)
                    tick["sell_quantity"] = struct.unpack(
                        ">I", data[offset : offset + 4]
                    )[0]
                    offset += 4
                    ...
                    tick["ohlc"] = {
                        ...
                        / 100,
                        "low": struct.unpack(">I", data[offset + 8 : offset + 12])[0]
                        / 100,
                        "close": struct.unpack(">I", data[offset + 12 : offset + 16])[0]
                        / 100,
                    }
                    offset += 16

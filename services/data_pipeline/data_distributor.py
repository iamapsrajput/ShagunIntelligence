...
class DataDistributor:
    ...
    def __init__(self, mode: DistributionMode = DistributionMode.BROADCAST):
        ...
        self.subscriber_metrics: Dict[str, SubscriberMetrics] = defaultdict(SubscriberMetrics)
    ...
    def add_subscriber(self, symbol: str, callback: Callable,
                      ...
                      priority: int = 5, batch_size: int = 1) -> str:
    ...
    async def _send_with_delay(self, subscriber: Subscriber, data: Dict[str, Any],
                              timestamp: float, delay: float) -> None:
    ...
    async def _send_to_subscriber(self, subscriber: Subscriber, data: Dict[str, Any],
                                 enqueue_timestamp: float) -> None:
    ...
    def _update_subscriber_metrics(self, subscriber_id: str, success: bool,
                                  latency_ms: float, processing_time_ms: float) -> None:
    ...
    def _start_batch_processor(self, subscriber_id: str) -> None:
        ...
        if subscriber_id not in self.batch_tasks:
            task = asyncio.create_task(
                self._batch_processor(subscriber_id)
            )

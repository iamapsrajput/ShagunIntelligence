...
class RedisCacheManager:
    ...
    async def cache_ohlc(
        self, symbol: str, interval: str, ohlc_data: list[dict[str, Any]]
    ) -> None:
        """Cache OHLC data"""
        if not self.redis_client:
            return
        ...
        try:
            ...
            await self.redis_client.setex(
                key, self.cache_config["ohlc_ttl"], serialized
            )
    ...
    async def save_buffer_snapshot(
        self, buffer_data: dict[str, list[dict[str, Any]]]
    ) -> None:
        ...
        try:
            ...
            # Save with TTL
            await self.redis_client.setex(
                key, self.cache_config["buffer_ttl"], serialized
            )
    ...
    async def get_latest_snapshot(self) -> dict[str, list[dict[str, Any]]] | None:
    ...
    async def get_buffer(self, symbol: str) -> list[dict[str, Any]] | None:
    ...
    async def get_stats(self, stats_type: str) -> dict[str, Any] | None:
        """Get statistics data"""
        if not self.redis_client:
            return None
        ...
        try:
            ...
            while True:
                cursor, keys = await self.redis_client.scan(
                    cursor, match="shagunintelligence:*", count=100
                )

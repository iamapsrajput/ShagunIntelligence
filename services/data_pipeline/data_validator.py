    ...
    def _validate_timestamp(
        ...
        try:
            ...
            if ts > now + timedelta(seconds=max_future):
                return (
                    False,
                    f"Timestamp is {(ts - now).total_seconds():.1f}s in the future",
                    None,
                )
    ...
    def _check_circuit_breaker(self, data: dict[str, Any]) -> bool:
        ...
        if instrument_token in self.last_good_values:
            ...
            price_change_percent = (
                abs((last_price - last_good_price) / last_good_price) * 100
            )

            # Check tick limit
            if price_change_percent > self.price_limits["tick_limit_percent"]:
                logger.warning(
                    f"Price movement {price_change_percent:.2f}% exceeds tick limit"
                )
                return False

        return True

    def _attempt_recovery(self, data: dict[str, Any]) -> dict[str, Any] | None:

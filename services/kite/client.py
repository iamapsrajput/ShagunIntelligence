...
from .order_management import OrderManager, OrderRequest, TransactionType, OrderType, ProductType
from .portfolio_manager import PortfolioManager
from .error_handler import global_error_handler, RetryConfig
from .validators import DataValidator
from .monitoring import monitoring_service
from .exceptions import KiteException, KiteAuthenticationError
...
class KiteConnectService:
    ...
    async def generate_session(self, request_token: str) -> Dict[str, Any]:
        ...
        monitoring_service.logger.log_api_call("generate_session", {"request_token": "***"})
        
        result = await self.auth_manager.generate_session(request_token)
        
        monitoring_service.logger.log_api_response("generate_session", len(str(result)), 0, True)
        return result
    ...
    async def get_quote(self, symbol: str, exchange: str = "NSE") -> Dict[str, Any]:
        ...
        monitoring_service.logger.log_api_call("get_quote", {"symbol": symbol, "exchange": exchange})
        
        try:
            instrument_key = f"{exchange}:{validation.value}"
            quote = await asyncio.to_thread(self.auth_manager.kite.quote, instrument_key)
            ...
            if instrument_key in quote:
                ...
                monitoring_service.logger.log_api_response("get_quote", len(str(result)), 0, True)
                return result
            else:
    ...
    async def get_quotes(self, symbols: List[str], exchange: str = "NSE") -> List[Dict[str, Any]]:
        """Get quotes for multiple symbols"""
        self._ensure_initialized()
        
        monitoring_service.logger.log_api_call("get_quotes", {"symbols_count": len(symbols), "exchange": exchange})
        ...
        try:
            ...
            for symbol in symbols:
                ...
                if validation.is_valid:
                    ...
                    logger.warning(f"Skipping invalid symbol {symbol}: {validation.errors}")
            
            if not validated_symbols:
                return []
            
            instrument_keys = [f"{exchange}:{symbol}" for symbol in validated_symbols]
            quotes = await asyncio.to_thread(self.auth_manager.kite.quote, instrument_keys)
            ...
            monitoring_service.logger.log_api_response("get_quotes", len(str(result)), 0, True)
            return result
            
        except Exception as e:
            monitoring_service.logger.log_error("get_quotes", e, {"symbols_count": len(symbols)})
            raise
    ...
    async def get_historical_data(
        ...
        try:
            interval_enum = Interval(interval.upper()) if hasattr(Interval, interval.upper()) else Interval.DAY
            ...
            return [
                {
                    "date": ohlc.timestamp.date() if hasattr(ohlc.timestamp, 'date') else ohlc.timestamp,
                    "open": ohlc.open,
                    "high": ohlc.high,
                    "low": ohlc.low,
                    "close": ohlc.close,
            ...
            monitoring_service.logger.log_error("get_historical_data", e, {"symbol": symbol})
            raise
    ...
    async def place_order(self, order_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        monitoring_service.logger.log_order_event("place_order_request", validation.value)
    ...
    async def cancel_order(self, order_id: str) -> bool:
        ...
        monitoring_service.logger.log_order_event("cancel_order_request", {"order_id": order_id})
        
        result = await self.order_manager.cancel_order(order_id)
        
        monitoring_service.logger.log_order_event("order_cancelled", {"order_id": order_id, "success": result})
        return result
        ...
        return [
            {
                ...
                "transaction_type": order.request.transaction_type.value if order.request else "",
    ...
    async def connect_websocket(self) -> bool:
        ...
        try:
            ...
            monitoring_service.logger.log_websocket_event("connect_response", {"success": success})
            return success
        except Exception as e:
    ...
    def _ensure_initialized(self):
        ...
        if not self.is_initialized:
            raise KiteAuthenticationError("Kite service not initialized. Please call initialize() first.")

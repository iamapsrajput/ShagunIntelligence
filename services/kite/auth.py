...
from .exceptions import KiteAuthenticationError
...
class KiteAuthManager:
    ...
    async def generate_session(self, request_token: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Session generated successfully for user: {self._user_profile.get('user_name', 'Unknown')}"
            )
    ...
    async def refresh_token_if_needed(self) -> bool:
        ...
        if self._token_expiry and datetime.now() >= (
            self._token_expiry - timedelta(hours=1)
        ):
            logger.warning(
                "Token is about to expire, manual re-authentication required"
            )
            await self._clear_stored_tokens()
            self._access_token = None

...
class OHLC:
    ...
    close: float
    volume: int
    oi: int | None = None  # Open Interest for derivatives
...
class HistoricalDataService:
    ...
    async def get_candlestick_data(
        ...
        """
        try:
            ohlc_data = await self.get_ohlc_data(
            ...
            logger.info(
                f"Created DataFrame with {len(df)} candlestick records for {symbol}"
            )
            return df
    ...
    async def _refresh_instruments_cache(self, exchange: str = None):
        ...
        try:
            logger.info(
                f"Refreshing instruments cache for {exchange or 'all exchanges'}"
            )

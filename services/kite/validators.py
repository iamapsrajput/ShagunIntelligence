...
class DataValidator:
    ...
    SYMBOL_PATTERNS = {
        ...
        Exchange.NFO: re.compile(
            r"^[A-Z0-9&\-]+\d{2}[A-Z]{3}\d{2}(CE|PE)\d+$|^[A-Z0-9&\-]+\d{2}[A-Z]{3}\d{2}FUT$"
        ),
        Exchange.BFO: re.compile(
            r"^[A-Z0-9&\-]+\d{2}[A-Z]{3}\d{2}(CE|PE)\d+$|^[A-Z0-9&\-]+\d{2}[A-Z]{3}\d{2}FUT$"
        ),
        ...
        try:
            ...
            if len(cleaned_symbol) < 1 or len(cleaned_symbol) > 30:
                result.errors.append(
                    "Symbol length must be between 1 and 30 characters"
                )
                return result
            ...
            try:
                ...
                if pattern and not pattern.match(cleaned_symbol):
                    result.errors.append(
                        f"Symbol '{cleaned_symbol}' doesn't match {exchange} format"
                    )
                    return result

            except ValueError:
                result.warnings.append(
                    f"Unknown exchange '{exchange}', skipping pattern validation"
                )
    ...
    @staticmethod
    def validate_price(
        price: float | int | str | Decimal, allow_zero: bool = False
    ) -> ValidationResult:
        ...
        try:
            ...
            if float_price > DataValidator.MAX_PRICE:
                result.errors.append(
                    f"Price {float_price} exceeds maximum limit {DataValidator.MAX_PRICE}"
                )
                return result

            if float_price > 0 and float_price < DataValidator.MIN_PRICE:
                result.errors.append(
                    f"Price {float_price} below minimum limit {DataValidator.MIN_PRICE}"
                )
                return result
            ...
            if decimal_price.as_tuple().exponent < -2:
                result.warnings.append(
                    "Price has more than 2 decimal places, will be rounded"
                )
    ...
    @staticmethod
    def validate_quantity(quantity: int | str) -> ValidationResult:
        ...
        try:
            ...
            if int_quantity < DataValidator.MIN_QUANTITY:
                result.errors.append(
                    f"Quantity {int_quantity} below minimum {DataValidator.MIN_QUANTITY}"
                )
                return result

            if int_quantity > DataValidator.MAX_QUANTITY:
                result.errors.append(
                    f"Quantity {int_quantity} exceeds maximum {DataValidator.MAX_QUANTITY}"
                )
                return result
    ...
    @staticmethod
    def validate_date(
        input_date: str | date | datetime, allow_future: bool = True
    ) -> ValidationResult:
        """Validate date input"""
        result = ValidationResult(is_valid=False)
    ...
    def validate_order_data(order_data: dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            symbol_result = DataValidator.validate_symbol(
                order_data["symbol"], order_data.get("exchange", "NSE")
            )
            ...
            if order_type in ["SL", "SL-M"] and "trigger_price" in order_data:
                trigger_result = DataValidator.validate_price(
    ...
    def validate_historical_data_request(
        ...
        try:
            ...
            # Validate from_date
            from_date_result = DataValidator.validate_date(
                from_date, allow_future=False
            )
            if not from_date_result.is_valid:
                result.errors.extend(
                    [f"From date: {err}" for err in from_date_result.errors]
                )
                result.is_valid = False
            else:
            ...
            if not to_date_result.is_valid:
                result.errors.extend(
                    [f"To date: {err}" for err in to_date_result.errors]
                )
                result.is_valid = False
            else:
            ...
            if result.is_valid:
                ...
                if date_diff > max_allowed:
                    result.warnings.append(
                        f"Date range ({date_diff} days) is large for {interval} interval"
                    )
...
class DataSanitizer:
    ...
    @staticmethod
    def sanitize_price(price: float | str | Decimal) -> float:
    ...
    @staticmethod
    def sanitize_quantity(quantity: int | str) -> int:
        ...
        try:
            ...
            for col in numeric_columns:
                if col in sanitized_df.columns:
                    ...
                    sanitized_df[col] = sanitized_df[col].replace(
            ...
            if all(
                col in sanitized_df.columns for col in ["open", "high", "low", "close"]
            ):
                # Ensure high >= max(open, close) and low <= min(open, close)
                sanitized_df["high"] = sanitized_df[["high", "open", "close"]].max(
                    axis=1
                )
    ...
    def sanitize_order_response(order_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            other_fields = set(order_data.keys()) - set(
                string_fields + numeric_fields + int_fields
            )

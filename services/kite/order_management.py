...
class OrderRequest:
    ...
    squareoff: Optional[float] = None  # For bracket orders
    stoploss: Optional[float] = None  # For bracket orders
    trailing_stoploss: Optional[float] = None
    tag: Optional[str] = None
    
    def __post_init__(self):
        # Validate order parameters
        if self.order_type in [OrderType.LIMIT, OrderType.STOP_LOSS] and self.price is None:
            raise KiteValidationError("Price is required for LIMIT and SL orders")
        
        if self.order_type in [OrderType.STOP_LOSS, OrderType.STOP_LOSS_MARKET] and self.trigger_price is None:
...
class OrderManager:
    ...
    def __init__(self, auth_manager: KiteAuthManager):
        ...
        self.rate_limiter = RateLimiter(max_requests=20, time_window=60)  # 20 orders per minute
    ...
    async def place_order(self, order_request: OrderRequest) -> str:
        ...
        try:
            ...
            kite_order_id = await asyncio.to_thread(
                self.auth_manager.kite.place_order,
                **order_params
            )
            ...
            logger.info(f"Order placed successfully: {kite_order_id} for {order_request.symbol}")
    ...
    async def place_stop_loss_order(
        ...
        order_type = OrderType.STOP_LOSS_MARKET if price is None else OrderType.STOP_LOSS
    ...
    async def modify_order(
        ...
        try:
            ...
            await asyncio.to_thread(
                self.auth_manager.kite.modify_order,
                order_id,
                **modify_params
            )
    ...
    async def cancel_order(self, order_id: str, variety: str = "regular") -> bool:
        ...
        try:
            ...
            await asyncio.to_thread(
                self.auth_manager.kite.cancel_order,
                variety,
                order_id
            )
    ...
    async def get_order_status(self, order_id: str) -> Optional[Order]:
        ...
        try:
            ...
            order_data = await asyncio.to_thread(
                self.auth_manager.kite.order_history,
                order_id
            )
    ...
    async def _validate_order(self, order_request: OrderRequest):
        ...
        if order_request.order_type == OrderType.BRACKET_ORDER:
            if order_request.squareoff is None or order_request.stoploss is None:
                raise KiteValidationError("Squareoff and stoploss are required for bracket orders")
        
        # Add more validations as needed
        logger.debug(f"Order validation passed for {order_request.symbol}")
    
    async def _prepare_order_params(self, order_request: OrderRequest) -> Dict[str, Any]:
        """Prepare order parameters for Kite API"""
        params = {
    ...
    async def _update_order_from_kite_data(self, order_id: str, kite_data: Dict[str, Any]):

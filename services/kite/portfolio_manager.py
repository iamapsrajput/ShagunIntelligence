...
class PortfolioManager:
    ...
    async def get_positions(self, refresh: bool = False) -> dict[str, list[Position]]:
        ...
        try:
            ...
            logger.info(
                f"Fetched {len(day_positions)} day positions and {len(net_positions)} net positions"
            )
    ...
    async def get_holdings(self, refresh: bool = False) -> list[Holding]:
        ...
        try:
            if not refresh and self._is_cache_valid():
                cached_holdings = [
                    holding
                    for key, holding in self._holdings_cache.items()
    ...
    async def get_portfolio_summary(self) -> PortfolioSummary:
        ...
        try:
            ...
            logger.info(
                f"Generated portfolio summary: Total value ₹{total_portfolio_value:,.2f}"
            )
            return summary
    ...
    async def get_position_by_symbol(
        self, symbol: str, position_type: PositionType = PositionType.NET
    ) -> Position | None:
    ...
    async def get_holding_by_symbol(self, symbol: str) -> Holding | None:
    ...
    async def get_margin_for_symbol(
        ...
        try:
            ...
            margin_data = await asyncio.to_thread(
                self.auth_manager.kite.margins, [instrument_token]
            )
    ...
    async def calculate_position_metrics(
        ...
        try:
            ...
            win_rate = (
                (len(profitable_positions) / len(positions)) * 100 if positions else 0
            )
    ...
    async def _create_holding_from_data(self, data: dict[str, Any]) -> Holding:
        ...
        if data.get("authorised_date"):
            try:
                authorised_date = datetime.strptime(
                    data["authorised_date"], "%Y-%m-%d"
                ).date()
            except:
                pass
    ...
    def _format_positions_response(self) -> dict[str, list[Position]]:
        ...
        day_positions = [
            pos for key, pos in self._positions_cache.items() if key.startswith("day:")
        ]
        net_positions = [
            pos for key, pos in self._positions_cache.items() if key.startswith("net:")
        ]

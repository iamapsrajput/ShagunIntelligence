...
class KiteErrorHandler:
            ...
            async def wrapper(*args, **kwargs):
                ...
                while retry_count <= config.max_retries:
                    try:
                        ...
                        last_exception = e
                        error_info = await self._process_error(
                            e, func.__name__, retry_count, args, kwargs
                        )
                        ...
                        if retry_count < config.max_retries:
                            ...
                            logger.warning(
                                f"Retrying {func.__name__} in {delay:.2f}s (attempt {retry_count + 1}/{config.max_retries})"
                            )
    ...
    async def _should_retry(
        self, exception: Exception, retry_count: int, config: RetryConfig
    ) -> bool:
        ...
        if isinstance(exception, ValueError | KeyError) and not isinstance(
            exception, KiteException
        ):
            return False
    ...
    async def _log_final_error(self, error_info: ErrorInfo | None):
        """Log final error after all retries exhausted"""
        if error_info:
            logger.error(
                f"Operation failed permanently after {error_info.retry_count} retries: {error_info.message}"
            )

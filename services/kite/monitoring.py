...
class KiteLogger:
    ...
    def log_api_call(self, operation: str, params: Dict[str, Any], level: LogLevel = LogLevel.INFO):
    ...
    def log_api_response(self, operation: str, response_size: int, duration: float, success: bool):
    ...
    def _sanitize_order_data(self, order_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        important_fields = [
            ...
            'price', 'order_type', 'status', 'average_price'
        ]
...
class MetricsCollector:
    ...
    def record_api_performance(self, metrics: PerformanceMetrics):
        ...
        with self._lock:
            ...
            total_duration = stats.avg_response_time * (stats.total_requests - 1) + metrics.duration
    ...
    def get_metrics_summary(self, time_window: int = 3600) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            recent_performance = [p for p in self.performance_history if p.start_time >= cutoff_time]
        ...
        if recent_performance:
            summary['avg_response_time'] = sum(p.duration for p in recent_performance) / len(recent_performance)
            ...
            for operation, perfs in operations.items():
                summary['operations'][operation] = {
                    ...
                    'error_rate': (sum(1 for p in perfs if not p.success) / len(perfs)) * 100
                }
        ...
        def aggregate_metrics():
            while True:
                try:
                    ...
                    with self._lock:
                        recent_calls = [p for p in self.performance_history if p.start_time >= cutoff_time]
        ...
        try:
            ...
            for operation, stats in api_stats.items():
                if stats['error_rate'] > 10:
                    health_score -= 10
                    issues.append(f"High error rate for {operation}: {stats['error_rate']:.1f}%")
            
            # Check response times
            if summary['avg_response_time'] > 3.0:
                health_score -= 20
                issues.append(f"Slow average response time: {summary['avg_response_time']:.2f}s")

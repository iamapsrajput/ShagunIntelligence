...
class RateLimiter:
    ...
    def __init__(self, max_requests_per_minute: int = 60, 
        ...
        logger.info(f"RateLimiter initialized - Requests: {max_requests_per_minute}/min, "
    ...
    async def check_request(self, provider: str, estimated_tokens: int) -> bool:
        ...
        async with self.lock:
            ...
            if recent_requests >= limits["requests_per_minute"]:
                ...
                self.stats[provider_name]["rate_limit_hits"] += 1
                logger.warning(f"Request rate limit exceeded for {provider_name}: "
            ...
            if recent_tokens > limits["tokens_per_minute"]:
                self.stats[provider_name]["blocked_requests"] += 1
                self.stats[provider_name]["rate_limit_hits"] += 1
                logger.warning(f"Token rate limit exceeded for {provider_name}: "
    ...
    async def record_request(self, provider: str, actual_tokens: int) -> None:
        ...
        async with self.lock:
            ...
            logger.debug(f"Recorded request for {provider_name}: {actual_tokens} tokens")
    ...
    def _clean_old_entries(self, provider: str, cutoff_time: float) -> None:
        ...
        while (self.request_history[provider] and 
               self.request_history[provider][0] < cutoff_time):
            self.request_history[provider].popleft()
        
        # Clean token history (paired with request history)
        while (len(self.token_history[provider]) > 
               len(self.request_history[provider])):
            self.token_history[provider].popleft()
    
    def _check_daily_limits(self, provider: str, estimated_tokens: int,
    ...
    async def wait_if_needed(self, provider: str, estimated_tokens: int) -> float:
        ...
        async with self.lock:
            ...
            if wait_time > 0:
                logger.info(f"Rate limited for {provider_name}, waiting {wait_time:.1f}s")
                await asyncio.sleep(wait_time)
            
            return wait_time
    
    def get_current_usage(self, provider: Optional[str] = None) -> Dict[str, Any]:
        ...
        if provider:
            ...
            recent_requests = sum(1 for t in self.request_history[provider_name] 
                                if t >= cutoff_time)
            recent_tokens = sum(tokens for t, tokens in 
                              zip(self.request_history[provider_name], 
            ...
            return {
                ...
                "requests": {
                    ...
                    "percentage": (recent_requests / limits["requests_per_minute"] * 100)
                },
                "tokens": {
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        for provider, stats in self.stats.items():
            ...
            # Aggregate totals
            for key in ["total_requests", "blocked_requests", "total_tokens", "rate_limit_hits"]:

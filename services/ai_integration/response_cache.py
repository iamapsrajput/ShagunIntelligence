...
class ResponseCache:
    ...
    def __init__(self, cache_dir: str = "data/ai_cache",
        ...
        logger.info(f"ResponseCache initialized - Memory: {max_memory_items} items, "
                   f"Disk: {'Enabled' if enable_disk_cache else 'Disabled'}")
    
    def _generate_cache_key(self, provider: str, use_case: str, 
        ...
        if cache_key in self.memory_cache:
            ...
            if self._is_entry_valid(entry, use_case):
                ...
                if "usage" in entry["response"]:
                    self.stats["total_saved_tokens"] += entry["response"]["usage"].get("total_tokens", 0)
        ...
        if self.enable_disk_cache:
            ...
            if disk_entry and self._is_entry_valid(disk_entry, use_case):
                ...
                if "usage" in disk_entry["response"]:
                    self.stats["total_saved_tokens"] += disk_entry["response"]["usage"].get("total_tokens", 0)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "disk_hit_rate": (self.stats["disk_hits"] / self.stats["hits"] * 100) if self.stats["hits"] > 0 else 0,
            "average_saved_tokens": (self.stats["total_saved_tokens"] / self.stats["hits"]) if self.stats["hits"] > 0 else 0,
            "average_saved_cost": (self.stats["total_saved_cost"] / self.stats["hits"]) if self.stats["hits"] > 0 else 0
        }

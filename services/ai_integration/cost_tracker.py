...
class CostTracker:
    ...
    def __init__(
        ...
        save_path: str | None = "data/ai_usage_logs",
    ):
        ...
        self.provider_costs = defaultdict(
            lambda: {
                "total_cost": 0.0,
                "total_tokens": 0,
                "request_count": 0,
        ...
        self.optimization_data = defaultdict(
            lambda: {
        ...
        logger.info(
            f"CostTracker initialized - Hourly: ${max_cost_per_hour}, "
    ...
    def check_budget(self, estimated_cost: float) -> bool:
        ...
        if hourly_total > self.max_cost_per_hour:
            logger.warning(
                f"Hourly budget would be exceeded: ${hourly_total:.2f}/${self.max_cost_per_hour}"
            )
            return False
        ...
        if daily_total > self.max_cost_per_day:
            logger.warning(
                f"Daily budget would be exceeded: ${daily_total:.2f}/${self.max_cost_per_day}"
            )
            return False
    ...
    def track_usage(
        ...
        if cost > 0.50:  # Log requests over $0.50
            logger.warning(
                f"High-cost request: {provider_name} - ${cost:.2f} for {use_case}"
            )
    ...
    def _update_optimization_data(self, provider: str, record: UsageRecord) -> None:
        ...
        if count > 0:
            ...
            opt_data["avg_tokens_per_request"] = (
                opt_data["avg_tokens_per_request"] * (count - 1) + record.total_tokens
            ) / count

            # Running average for cost
            opt_data["avg_cost_per_request"] = (
                opt_data["avg_cost_per_request"] * (count - 1) + record.cost
            ) / count
    ...
    def get_current_usage(self) -> dict[str, Any]:
        ...
        return {
            "hourly": {
                ...
                "percentage": (
                    self.hourly_costs[current_hour] / self.max_cost_per_hour * 100
                ),
                "remaining": max(
                    0, self.max_cost_per_hour - self.hourly_costs[current_hour]
                ),
            },
            "daily": {
                ...
                "percentage": (
                    self.daily_costs[current_day] / self.max_cost_per_day * 100
                ),
                "remaining": max(
                    0, self.max_cost_per_day - self.daily_costs[current_day]
                ),
            },
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        for provider, data in self.provider_costs.items():
            provider_summary[provider] = {
                ...
                "average_cost": (
                    data["total_cost"] / data["request_count"]
    ...
    def _get_hourly_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "peak_hour": (
                max(hourly_data, key=lambda x: x["cost"]) if hourly_data else None
            ),
            "average_hourly": (
                sum(h["cost"] for h in hourly_data) / len(hourly_data)
                if hourly_data
                else 0
            ),
        }
    ...
    def _get_daily_summary(self) -> dict[str, Any]:
        ...
        for i in range(7):
            ...
            daily_data.append(
                {"date": day_key, "cost": self.daily_costs.get(day_key, 0)}
            )
        ...
        return {
            ...
            "average_daily": (
                sum(d["cost"] for d in daily_data) / len(daily_data)
                if daily_data
                else 0
            ),
        }
    ...
    def _get_top_items(
        self, items: dict[str, float], limit: int
    ) -> list[dict[str, Any]]:
        """Get top items by cost"""
        sorted_items = sorted(items.items(), key=lambda x: x[1], reverse=True)

        return [{"name": name, "cost": cost} for name, cost in sorted_items[:limit]]
    ...
    def save_to_file(self, filename: str | None = None) -> None:
        ...
        data = {
            "metadata": {
                ...
                "date_range": {
                    "start": (
                        min(r.timestamp for r in self.usage_records).isoformat()
                        if self.usage_records
                        else None
                    ),
                    "end": (
                        max(r.timestamp for r in self.usage_records).isoformat()
                        if self.usage_records
                        else None
                    ),
                },
    ...
    def reset_daily_budget(self) -> None:
        ...
        self.daily_costs = {
            k: v for k, v in self.daily_costs.items() if k >= cutoff_date
        }

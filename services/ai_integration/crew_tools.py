...
from services.ai_integration.ai_service_manager import AIServiceManager, AIProvider
...
class MarketSentimentTool(BaseTool):
    ...
    def _run(self, text: str, context: Optional[Dict[str, Any]] = None,
    ...
    def _generate_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if confidence > 0.7:
            if score > 0.5:
                recommendations.append("Consider bullish positions with proper risk management")
...
class TradeAnalysisTool(BaseTool):
    ...
    def _calculate_trade_score(self, analysis: Dict[str, Any]) -> float:
        ...
        if conditions.get("trend") == "uptrend" and analysis.get("recommendation") == "buy":
            score += 15
        elif conditions.get("trend") == "downtrend" and analysis.get("recommendation") == "sell":
            score += 15
...
class NewsAnalysisTool(BaseTool):
    ...
    def _run(self, articles: List[str], max_words: int = 500,
        ...
        try:
            ...
            structured_result = {
                ...
                "trading_signals": self._extract_trading_signals(result.get("summary", "")),
                "metadata": {
            ...
            return json.dumps({
                "error": str(e),
                "summary": "Failed to analyze news articles"
            })
        finally:
            loop.close()
    ...
    def _extract_trading_signals(self, summary: str) -> List[Dict[str, str]]:
        ...
        if "miss expectations" in summary.lower():
            signals.append({
                "signal": "bearish",
                "reason": "Earnings miss expectations"
            })
...
class RiskAssessmentTool(BaseTool):
    ...
    def _run(self, trade_data: Dict[str, Any], provider: str = "openai") -> str:
        ...
        try:
            ...
            result["risk_management"] = self._generate_risk_management(result, trade_data)
            result["position_sizing_calculator"] = self._calculate_position_size(result, trade_data)
    ...
    def _generate_risk_management(self, assessment: Dict[str, Any], 
    ...
    def _calculate_position_size(self, assessment: Dict[str, Any],
                               trade_data: Dict[str, Any]) -> Dict[str, Any]:

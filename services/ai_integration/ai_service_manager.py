...
class AIServiceManager:
    ...
    async def analyze_market_sentiment(
        self, text: str, provider: AIProvider | None = None
    ) -> dict[str, Any]:
    ...
    async def generate_trade_analysis(
        self, market_data: dict[str, Any], provider: AIProvider | None = None
    ) -> dict[str, Any]:
        """Generate comprehensive trade analysis"""
        prompt = self.prompt_templates.get_trade_analysis_prompt(market_data)

        response = await self._execute_request(
            prompt=prompt, provider=provider, use_case="trade_analysis", max_tokens=1000
        )

        return self._parse_trade_analysis_response(response)

    async def summarize_news(
        self, articles: list[str], provider: AIProvider | None = None
    ) -> str:
        ...
        response = await self._execute_request(
            prompt=prompt, provider=provider, use_case="news_summary", max_tokens=500
        )

        return response.get("content", "")

    async def extract_key_metrics(
        self, financial_text: str, provider: AIProvider | None = None
    ) -> dict[str, Any]:
    ...
    async def generate_risk_assessment(
        self, trade_data: dict[str, Any], provider: AIProvider | None = None
    ) -> dict[str, Any]:
        """Generate AI-powered risk assessment"""
        prompt = self.prompt_templates.get_risk_assessment_prompt(trade_data)

        response = await self._execute_request(
            prompt=prompt, provider=provider, use_case="risk_assessment", max_tokens=800
        )
    ...
    async def _execute_request(
        ...
        if not await self.rate_limiter.check_request(provider, max_tokens):
            ...
            if self.config.enable_fallback:
                ...
                if not provider or not await self.rate_limiter.check_request(
                    provider, max_tokens
                ):
        ...
        try:
            ...
            await self.rate_limiter.record_request(
                provider, response.get("usage", {}).get("total_tokens", max_tokens)
            )
            ...
            if self.config.enable_fallback:
                ...
                if (
                    fallback_provider
                    and self.service_health[fallback_provider]["available"]
                ):
    ...
    def _get_fallback_provider(self, primary: AIProvider) -> AIProvider | None:
        """Get fallback provider"""
        if primary == AIProvider.OPENAI:
            return (
                AIProvider.ANTHROPIC if AIProvider.ANTHROPIC in self.clients else None
            )
        else:
            return AIProvider.OPENAI if AIProvider.OPENAI in self.clients else None

    def _estimate_cost(
        self, provider: AIProvider, prompt: str, max_tokens: int
    ) -> float:
    ...
    def _parse_trade_analysis_response(
    ...
    def _parse_metrics_response(self, response: dict[str, Any]) -> dict[str, Any]:
        ...
        result = {"metrics": {}, "raw_response": content}

...
def main():
    ...
    if args.coverage:
        base_cmd += (
            " --cov=app --cov=agents --cov=services --cov-report=html --cov-report=term"
        )
    ...
    test_suites = {
        "unit": [
            (
                ...
                f"{base_cmd} tests/unit/test_market_analyst.py",
            ),
            (
                "Unit Tests - Risk Manager",
                f"{base_cmd} tests/unit/test_risk_manager.py",
            ),
            (
                "Unit Tests - Trade Executor",
                f"{base_cmd} tests/unit/test_trade_executor.py",
            ),
        ],
        "integration": [
            (
                "Integration Tests - Historical Data",
                f"{base_cmd} tests/integration/test_historical_data_integration.py",
            ),
        ],
        "performance": [
            (
        ...
        "validation": [
            (
                ...
                f"{base_cmd} tests/validation/test_risk_management.py",
            ),
        ],
        "simulation": [
            (
                "Paper Trading Simulation",
                f"{base_cmd} tests/simulation/test_paper_trading.py",
            ),
        ],
    }
    ...
    # Save results
    results_file = (
        report_dir / f"test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
    ...
    # Flake8 linting
    run_command(
        "flake8 --max-line-length=100 --exclude=venv,__pycache__ .", "Linting (Flake8)"
    )

    # Type checking with mypy
    run_command(
        "mypy app/ agents/ services/ --ignore-missing-imports", "Type Checking (MyPy)"
    )

...
async def get_api_configuration(
    ...
    if provider:
        ...
        return provider_config.dict(exclude={'api_key', 'api_secret', 'access_token', 'bearer_token'})
...
async def update_api_key(
    ...
    return {"status": "success", "message": f"Updated {key_update.key_type} for {key_update.provider.value}"}
...
async def get_failover_priority(
    feature: str = Query(..., description="Feature: websocket, historical, realtime, news, sentiment"),
    current_user: dict = Depends(get_current_user)
):
    ...
    if not apis:
        raise HTTPException(status_code=404, detail=f"No APIs found for feature: {feature}")
...
async def get_management_dashboard(
    ...
    dashboard = {
        ...
        'api_status': {
            ...
            'configured': len([
                p for p in APIProvider
                if any(key_manager.get_key(p, kt) for kt in ['api_key', 'api_secret', 'access_token', 'bearer_token'])
            ])
        },
        ...
        'rate_limits': {
            provider: {
                ...
                'utilization': {
                    'minute': (info['current']['minute'] / info['limits']['per_minute'] * 100) 
                             if info['limits']['per_minute'] else 0,
                    'hour': (info['current']['hour'] / info['limits']['per_hour'] * 100)
                           if info['limits']['per_hour'] else 0,
                    'day': (info['current']['day'] / info['limits']['per_day'] * 100)
                          if info['limits']['per_day'] else 0
                }
            }
        ...
        'key_rotation': {
            'overdue': len([
                s for s in key_manager.get_rotation_schedule()
                if s['status'] == 'overdue'
            ]),
            'due_soon': len([
                s for s in key_manager.get_rotation_schedule()
...
async def export_env_template(
    ...
    return {"status": "success", "filename": filename, "message": "Template created. Copy to .env and add your API keys."}
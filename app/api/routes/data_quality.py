...
async def get_source_reliability(
    ...
    try:
        data_integration = get_data_source_integration()
        report = (
            data_integration.manager._quality_validator.get_source_reliability_report()
        )
...
async def get_quality_alerts(
    ...
    severity: QualityGrade | None = Query(None, description="Filter by severity"),
    current_user: User = Depends(get_current_user),
) -> list[dict[str, Any]]:
    """
...
async def update_quality_threshold(
    alert_threshold: float = Query(
        ..., ge=0.0, le=1.0, description="Quality score threshold for alerts"
    ),
    current_user: User = Depends(get_current_user),
) -> dict[str, Any]:
    """
...
def _calculate_report_summary(report: dict[str, Any]) -> dict[str, Any]:
    ...
    reliability_scores = [
        source.get("reliability_score", 0.0) for source in source_reliability.values()
    ]
    average_reliability = (
...
async def get_data_quality_metrics(
    ...
    lookback_minutes: int = Query(
        default=30, description="Minutes to look back for metrics"
    ),
    current_user: User = Depends(get_current_user),
):
    ...
    try:
        ...
        high_quality_sources = sum(
            1
            for s in quality_data["sources"].values()
...
async def get_multi_source_health_status(
    ...
    try:
        ...
        healthy_sources = sum(
            1 for s in health_data["sources"].values() if s["status"] == "healthy"
        )
        ...
        return {
            ...
            "summary": {
                ...
                "degraded_sources": sum(
                    1
                    for s in health_data["sources"].values()
                ...
                "overall_health_score": (
                    (healthy_sources / total_sources * 100) if total_sources > 0 else 0
                ),
            },
            "api_usage": api_costs,
        }
...
async def get_sentiment_fusion(
    ...
    lookback_hours: int = Query(
        default=24, description="Hours to look back for sentiment"
    ),
    current_user: User = Depends(get_current_user),
):
    ...
    try:
        ...
        consensus = {
            "bullish": sum(
                1 for s in source_sentiments.values() if s["sentiment"] > 0.3
            ),
            "neutral": sum(
                1 for s in source_sentiments.values() if -0.3 <= s["sentiment"] <= 0.3
            ),
            "bearish": sum(
                1 for s in source_sentiments.values() if s["sentiment"] < -0.3
            ),
    ...
    try:
        ...
        return {
            ...
            "summary": {
                ...
                "active_streams": sum(
                    1 for s in stream_status.values() if s["status"] == "connected"
                ),
                "disconnected_streams": sum(
                    1 for s in stream_status.values() if s["status"] == "disconnected"
                ),
                "error_streams": sum(
                    1 for s in stream_status.values() if s["status"] == "error"
                ),
            },
        }
...
async def get_api_cost_summary(
    ...
    try:
        ...
        if period == "day":
            ...
            for _provider, data in cost_report["providers"].items():

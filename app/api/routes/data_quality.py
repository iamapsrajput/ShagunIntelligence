...
from agents.base_quality_aware_agent import DataQualityLevel
...
async def validate_symbol_data(symbol: str, current_user: User = Depends(get_current_user)) -> Dict[str, Any]:
    """
...
async def get_best_quality_quote(symbol: str, current_user: User = Depends(get_current_user)) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {"symbol": symbol, "quote": quote, "source": quote.get("source", "unknown")}
...
@router.get("/report")
async def get_quality_report(current_user: User = Depends(get_current_user)) -> Dict[str, Any]:
    """
...
async def update_quality_threshold(
    ...
    try:
        ...
        return {"message": "Alert threshold updated successfully", "new_threshold": alert_threshold}
...
def _calculate_report_summary(report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    healthy_sources = sum(1 for source in source_reliability.values() if source.get("grade") in ["Excellent", "Good"])

    reliability_scores = [source.get("reliability_score", 0.0) for source in source_reliability.values()]
...
async def get_data_quality_metrics(
    ...
    try:
        ...
        historical_quality = await data_manager.get_historical_quality(symbol, start_time=cutoff_time)

        # Calculate aggregated metrics
        total_sources = len(quality_data["sources"])
        high_quality_sources = sum(
            1 for s in quality_data["sources"].values() if s["quality_level"] == DataQualityLevel.HIGH
        )

        # Get recent failover events
        failover_events = await data_manager.get_failover_events(symbol, start_time=cutoff_time)
...
async def get_multi_source_health_status(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        healthy_sources = sum(1 for s in health_data["sources"].values() if s["status"] == "healthy")
        ...
        return {
            ...
            "summary": {
                "total_sources": total_sources,
                "healthy_sources": healthy_sources,
                "degraded_sources": sum(1 for s in health_data["sources"].values() if s["status"] == "degraded"),
                "unhealthy_sources": sum(1 for s in health_data["sources"].values() if s["status"] == "unhealthy"),
...
async def get_sentiment_fusion(
    ...
    try:
        ...
        historical_sentiment = await data_manager.get_historical_sentiment(symbol, start_time=cutoff_time)
        ...
        return {
            ...
            "metadata": {
                ...
                "high_quality_sources": sum(1 for s in source_sentiments.values() if s.get("quality_score", 0) > 0.8),
...
async def get_failover_logs(
    ...
    try:
        ...
        events = await data_manager.get_failover_logs(start_time=start_time, end_time=end_time, limit=limit)
        ...
        while True:
            ...
            if data["type"] == "subscribe":
                ...
                if symbol:
                    ...
                    quality_data = await data_manager.get_quality_metrics(symbol)
                    await websocket.send_json({"type": "quality_update", "symbol": symbol, "data": quality_data})
...
# asyncio.create_task(broadcast_quality_updates())  # Commented out to avoid event loop issues at import time


@router.get("/stream-health")
async def get_stream_health(current_user: User = Depends(get_current_user)):

...
class APIKeyManager:
    ...
    def __init__(self, storage_path: str = None, master_password: str = None):
        self.storage_path = Path(
            storage_path or os.getenv("API_KEY_STORAGE", "./config/api_keys.enc")
        )
            ...
            for provider, keys in data.items():
                ...
                for key_type, record_data in keys.items():
                    ...
                    if record_data.get("expires_at"):
                        record_data["expires_at"] = datetime.fromisoformat(
                            record_data["expires_at"]
                        )
                    if record_data.get("last_rotated"):
                        record_data["last_rotated"] = datetime.fromisoformat(
                            record_data["last_rotated"]
                        )
                    if record_data.get("last_used"):
                        record_data["last_used"] = datetime.fromisoformat(
                            record_data["last_used"]
                        )
    ...
    def get_key(self, provider: APIProvider, key_type: str) -> str | None:
    ...
    async def rotate_key(
        ...
        try:
            ...
            if provider_str not in self.keys or key_type not in self.keys[provider_str]:
                logger.error(
                    f"Cannot rotate non-existent key: {provider_str}/{key_type}"
                )
                return False
    ...
    def get_all_keys_status(self) -> dict[str, Any]:
        ...
        for provider in APIProvider:
            provider_str = provider.value
            provider_status = {"enabled": False, "keys": {}}
            ...
            for key_type in ["api_key", "api_secret", "access_token", "bearer_token"]:
                ...
                # Check environment
                env_prefix = get_api_config().Config.env_prefix_map.get(
                    provider_str, ""
                )
                env_var = f"{env_prefix}{key_type.upper()}"
    ...
    def get_rotation_schedule(self) -> list[dict[str, Any]]:
        ...
        for provider, keys in self.keys.items():
            for key_type, record in keys.items():
                ...
                rotation_info = {
                    ...
                    "key_type": key_type,
                    "last_rotated": (
                        record.last_rotated.isoformat() if record.last_rotated else None
                    ),
                    "next_rotation": None,
                ...
                if record.last_rotated:
                    next_rotation = record.last_rotated + timedelta(
        ...
        return sorted(
            schedule, key=lambda x: x.get("days_until_rotation", float("inf"))
        )
...
# Singleton instance
_key_manager_instance: APIKeyManager | None = None


def get_api_key_manager(
    storage_path: str = None, master_password: str = None
) -> APIKeyManager:

...
class TradingScheduler:
    ...
    def _setup_jobs(self):
        ...
        self.scheduler.add_job(
            ...
            trigger=CronTrigger(
                day_of_week="mon-fri", hour="9-15", minute="*", timezone="Asia/Kolkata"
            ),
        ...
        self.scheduler.add_job(
            ...
            trigger=CronTrigger(
                day_of_week="mon-fri", hour=8, minute=30, timezone="Asia/Kolkata"
            ),
        ...
        self.scheduler.add_job(
            ...
            trigger=CronTrigger(
                day_of_week="mon-fri", hour=9, minute=15, timezone="Asia/Kolkata"
            ),
            id="market_open",
        ...
        self.scheduler.add_job(
            ...
            trigger=CronTrigger(
                day_of_week="mon-fri", hour=15, minute=30, timezone="Asia/Kolkata"
            ),
            id="market_close",
        ...
        self.scheduler.add_job(
            ...
            trigger=CronTrigger(
                day_of_week="mon-fri", hour=16, minute=0, timezone="Asia/Kolkata"
            ),
            id="daily_report",
        ...
        self.scheduler.add_job(
            ...
            trigger=CronTrigger(
                day_of_week="sun", hour=22, minute=0, timezone="Asia/Kolkata"
            ),
    ...
    async def _check_market_hours(self):
        ...
        try:
            ...
            if (
                hasattr(self.app_state, "market_open")
                and self.app_state.market_open != is_market_open
            ):
                self.app_state.market_open = is_market_open

                await websocket_broadcaster.broadcast_system_status(
                    {"market_open": is_market_open, "timestamp": now.isoformat()}
                )

                logger.info(
                    f"Market status changed: {'OPEN' if is_market_open else 'CLOSED'}"
                )
    ...
    async def _pre_market_analysis(self):
        ...
        try:
            ...
            watchlist = self.app_state.watchlist or [
                "RELIANCE",
                "TCS",
                "INFY",
                "HDFC",
            ...
            for symbol in watchlist:
                try:
                    ...
                    logger.info(
                        f"Pre-market analysis for {symbol}: {analysis.get('recommendation', 'N/A')}"
                    )
                except Exception as e:
            ...
            for position in positions.get("net", []):
                if position["quantity"] != 0:
                    ...
                    if exit_analysis.get("should_exit", False):
                        logger.warning(
                            f"Exit signal for {position['tradingsymbol']}: {exit_analysis.get('reason')}"
                        )
    ...
    async def _generate_daily_report(self):
        ...
        try:
            ...
            successful_trades = len(
                [o for o in orders if o.get("status") == "COMPLETE"]
            )
        ...
        try:
            ...
            if hasattr(self.app_state, "kite_client"):
                health_status["components"]["kite_api"] = (
                    "healthy"
                    if self.app_state.kite_client.is_connected()
                    else "unhealthy"
                )
            ...
            if hasattr(self.app_state, "crew_manager"):
                agents_status = (
                    await self.app_state.crew_manager.get_all_agents_status()
                )

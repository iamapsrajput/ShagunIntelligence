...
from .api_config import get_api_config, APIProvider, Environment
from .api_key_manager import get_api_key_manager
from .api_rate_limiter import get_rate_limiter
from .api_health_monitor import get_health_monitor, HealthAlert, AlertSeverity
        ...
        if tasks:
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    logger.error(f"Health check failed for {critical_providers[i].value}: {result}")
                else:
                    logger.info(
...
# Singleton instance
_api_system_instance: Optional[APIManagementSystem] = None

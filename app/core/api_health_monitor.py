...
from .api_config import APIConfig, APIProvider, get_api_config
...
class APIHealthMonitor:
    ...
    async def _perform_health_check(
        self, provider: APIProvider, config: APIConfig
    ) -> HealthCheckResult:
        ...
        try:
            ...
            async with self.session.get(url, headers=headers) as response:
                ...
                if response.status == 200:
                    ...
                    )  # Client errors don't indicate API health
                else:
    ...
    async def _analyze_health_status(self):
        ...
        for provider_str, history in self.health_history.items():
            ...
            response_times = [
                r.response_time_ms for r in recent_results if r.response_time_ms > 0
            ]
    ...
    async def _generate_health_alert(
        ...
        # Set cooldown
        self.alert_cooldown[provider] = datetime.now() + timedelta(
    ...
    def get_health_status(self, provider: APIProvider | None = None) -> dict[str, Any]:
    ...
    def _get_provider_health_status(self, provider: str) -> dict[str, Any]:
        ...
        return {
            ...
            "last_check": (
                self.last_check.get(provider).isoformat()
                if provider in self.last_check
                else None
            ),
            "metrics": metrics,
        }
    ...
    def get_health_report(self) -> dict[str, Any]:
        ...
        report = {
            ...
            "summary": {
                ...
                "healthy": sum(
                    1 for s in self.current_status.values() if s == HealthStatus.HEALTHY
                ),
                "degraded": sum(
                    1
                ...
                "unknown": sum(
                    1 for s in self.current_status.values() if s == HealthStatus.UNKNOWN
                ),
            },
            "recent_alerts": [
                {
        ...
        if total > 0:
            ...
            report["summary"]["health_score"] = max(
                0, healthy_score - degraded_penalty - unhealthy_penalty
            )
        else:
...
# Singleton instance
_health_monitor_instance: APIHealthMonitor | None = None


def get_health_monitor(alert_callback: Callable | None = None) -> APIHealthMonitor:

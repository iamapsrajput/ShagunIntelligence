...
from backend.streaming.agent_integration import AgentNotification, StreamingAgentBridge
from backend.streaming.performance_monitor import (
    PerformanceAlert,
...
async def performance_alert_handler(alert: PerformanceAlert):
    ...
    logger.warning(f"PERFORMANCE ALERT [{alert.severity.upper()}]: " f"{alert.message}")
...
async def demonstrate_streaming_system():
    ...
    try:
        ...
        for stream, result in test_result["streams"].items():
            if result["success"]:
                print(
                    f"   {stream}: Quality={result['quality_score']:.2f}, "
        ...
        if multi_data["streams"]:
            ...
            for stream, data in multi_data["streams"].items():
                print(
                    f"   {stream}: Price={data.get('price', 'N/A')}, "
        ...
        print(
            f"   Total Messages: {perf_report['current_metrics']['summary']['total_messages']}"
        )
        print(
            f"   Error Rate: {perf_report['current_metrics']['summary']['overall_error_rate']:.1%}"
        )

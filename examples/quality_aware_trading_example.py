...
from agents.coordinator.agent import AgentType, CoordinatorAgent
...
async def demonstrate_quality_aware_trading():
    ...
    quality_level = (
        ...
        if aggregated_data["quality"] > 0.8
        else (
            DataQualityLevel.MEDIUM
            if aggregated_data["quality"] > 0.6
    ...
    print(
        f"   Trading Mode: {TradingMode.AGGRESSIVE.value if quality_level == DataQualityLevel.HIGH else TradingMode.NORMAL.value}"
    )
    ...
    TradeExecutorAgent(paper_trading=True)
    ...
    signal = TradeSignal(
        ...
        price=(
            aggregated_data["data"]["current_price"]
            if quality_level != DataQualityLevel.HIGH
            else None
        ),
    ...
    if consensus_data:
        ...
        print(
            f"   Reliable Sources: {consensus_data.get('reliable_sources', 0)}/{consensus_data.get('total_sources', 0)}"
        )
    ...
    print(
        f"   Position Size: ₹{position_value:,.0f} ({adjusted_position_size:.1%} of capital)"
    )
    print(f"   Order Type: {signal.order_type}")
    ...
    if quality_level == DataQualityLevel.HIGH:
        ...
        print(
            "   Decision: ⚠️ EXECUTE WITH CAUTION - Reduced size due to medium quality"
        )
    else:
